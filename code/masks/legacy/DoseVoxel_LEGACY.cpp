	...
	namespace masks
	{
		namespace legacy
		{
			...
			DoseVoxel::DoseVoxel(const VoxelGridIndex3D& aDoseIndex, const core::GeometricInfo* aGeometricInfo)
			{
				...
				else if (aGeometricInfo->getPixelSpacingRow() <= 0 || aGeometricInfo->getPixelSpacingColumn() <= 0
				         || aGeometricInfo->getSliceThickness() <= 0)
				{
			...
			DoseVoxel::DoseVoxel(const LegacyDoseVoxelIndex3D& aDoseIndex,
			                     const rttb::core::GeometricInfo* aGeometricInfo)
			{
			...
			/*! @brief Transform dose index to world coordinate
			     *
			     */
			void DoseVoxel::setDoseVoxel(const VoxelGridIndex3D& aDoseIndex,
			...
			void DoseVoxel::setDoseVoxel(const LegacyDoseVoxelIndex3D& aDoseIndex,
			                             const rttb::core::GeometricInfo* aGeometricInfo)
			{
			...
			void DoseVoxel::setDoseVoxel(LegacyWorldCoordinate3D aWorldCoordinate,
			                             LegacyWorldCoordinate3D aVoxelDelta)
			{
			...
			*/
			...
			double DoseVoxel::voxelInStructureAtIndex(const StructureLegacy& aRTStructure,
			        int polygonIndex) const
			{
			...
			int DoseVoxel::getCorrespondingSlice(const StructureLegacy& aRTStructure) const
			{
				...
				const LegacyPolygonSequenceType& strVector =
				    aRTStructure.getLegacyStructureVector(); //strVector is ranked by _z of the contour
				int size = strVector.size();
			...
			std::vector<int> DoseVoxel::getZIntersectPolygonIndexVector(const StructureLegacy& aRTStructure)
			const
			{
			...
			std::vector<WorldCoordinate3D> DoseVoxel::calcIntersectionPoints(WorldCoordinate3D firstPoint,
			        WorldCoordinate3D secondPoint)
			{
				...
				if (a == 0)
				{
					if (firstPoint(0) >= _x && firstPoint(0) <= (_x + _deltaX))
					{
						...
						if (min(firstPoint(1), secondPoint(1)) <= (_y + _deltaY)
						    && max(firstPoint(1), secondPoint(1)) >= (_y + _deltaY))
						{
							WorldCoordinate3D p;
							p(0) = firstPoint(0);
							p(1) = _y + _deltaY;
				...
				else if (b == 0)
				{
					if (firstPoint(1) >= _y && firstPoint(1) <= (_y + _deltaY))
					{
						...
						if (min(firstPoint(0), secondPoint(0)) <= (_x + _deltaX)
						    && max(firstPoint(0), secondPoint(0)) >= (_x + _deltaX))
						{
							WorldCoordinate3D p;
							p(0) = _x + _deltaX;
							p(1) = firstPoint(1);
			...
			std::vector<LegacyWorldCoordinate3D> DoseVoxel::calcIntersectionPoints(
			    LegacyWorldCoordinate3D firstPoint, LegacyWorldCoordinate3D secondPoint)
			{
				...
				if (a == 0)
				{
					if (firstPoint.x >= _x && firstPoint.x <= (_x + _deltaX))
					{
						...
						if (min(firstPoint.y, secondPoint.y) <= (_y + _deltaY)
						    && max(firstPoint.y, secondPoint.y) >= (_y + _deltaY))
						{
				...
				else if (b == 0)
				{
					if (firstPoint.y >= _y && firstPoint.y <= (_y + _deltaY))
					{
						...
						if (min(firstPoint.x, secondPoint.x) <= (_x + _deltaX)
						    && max(firstPoint.x, secondPoint.x) >= (_x + _deltaX))
						{
