		...
		namespace legacy
		{
			...
			*/
			...
			public:
				...
				BaseField(Uint16 dimx_in , Uint16 dimy_in, Uint16 dimz_in): dimx(dimx_in), dimy(dimy_in),
					dimz(dimz_in) {}

				/*! @return  extend of the field in x-direction.
				*/
				virtual Uint16 GetDimX()
				{
					return dimx;
				}
			...
			*/
			...
			public:
				...
				*/
				FieldOfScalar(Uint16 dimx_in , Uint16 dimy_in, Uint16 dimz_in): BaseField(dimx_in , dimy_in ,
					        dimz_in)
				{
					create();
				}
				...
				*/
				bool CheckOneOutOfThisBlockHasSameIntensity(Uint16 x, Uint16 y, Uint16 z , Uint16 block_length ,
				        TYPE value);
			...
			template <class TYPE>
			bool FieldOfScalar<TYPE>::CheckBlockSameIntensity(Uint16 x, Uint16 y, Uint16 z ,
			        Uint16 block_length , TYPE value)
			{
				if (x >= dimx)
				{
					assert(0);
			...
			template <class TYPE>
			void FieldOfScalar<TYPE>::SetBlockThisIntensity(Uint16 x, Uint16 y, Uint16 z , Uint16 block_length ,
			        TYPE value)
			{
				if (x >= dimx)
				{
						...
						for (GridIndexType x = av.at(z).index_begin.x ; x <= av.at(z).index_end.x ; x++)
						{
							...
							if (*data_pointer == unclear_intensity)
							{
								...
								if (x > av.at(z).index_begin.x)
								{
									...
									if (*data_pointer_check_neighbour  == inside_intensity)
									{
										its_inside = true;
									}

									if (*data_pointer_check_neighbour  == outside_intensity)
									{
										its_outside = true;
									}
								}
			...
			template <class TYPE>
			bool FieldOfScalar<TYPE>::CheckOneOutOfThisBlockHasSameIntensity(Uint16 x, Uint16 y, Uint16 z ,
			        Uint16 block_length ,
			        TYPE value)
			{
				if (x >= dimx)
			...
			*/
			...
			{
			public:
				FieldOfPointer(Uint16 dimx_in , Uint16 dimy_in, Uint16 dimz_in): BaseField(dimx_in , dimy_in ,
					        dimz_in)
				{
					create();
				}
