...
namespace rttb{
  namespace masks{
    ...
    void PolygonInfo::CalcSnippetVectors(){
      ...
      for( int voxel_edge = 0 ; voxel_edge < 4 ; voxel_edge++ ){ 
        ...
        while( finished == false ){
          finished = AppendNextIntersectionAndCorrespoindingPolygonSnippets( snip_vec_ref_local, snip_vec_ref , voxel_edge, next_voxel_intersection );
        } 
    ...
    void PolygonInfo::ShowSnippet( PolygonSnippet a_snippet ){
      ...
      std::cout<< " a_snippet.point_of_interest_start.x : " << a_snippet.point_of_interest_start.x  <<std::endl;
      std::cout<< " a_snippet.point_of_interest_start.y : " << a_snippet.point_of_interest_start.y  <<std::endl;

      std::cout<< " a_snippet.point_of_interest_end.x : " << a_snippet.point_of_interest_end.x  <<std::endl;
      std::cout<< " a_snippet.point_of_interest_end.y : " << a_snippet.point_of_interest_end.y  <<std::endl;  

      std::cout<< " a_snippet.snippet_intermediate_content.size() : " << a_snippet.snippet_intermediate_content.size()  <<std::endl; 

      for( unsigned int counter = 0 ; counter < a_snippet.snippet_intermediate_content.size() ; counter++ ){
        std::cout<< " a_snippet.snippet_intermediate_content.at( counter ).x : " << a_snippet.snippet_intermediate_content.at( counter ).x  <<std::endl;
        std::cout<< " a_snippet.snippet_intermediate_content.at( counter ).y : " << a_snippet.snippet_intermediate_content.at( counter ).y  <<std::endl;
      }    
    } 
    ...
    bool PolygonInfo::TestCreateUnifiedListOfRawAndColumnIntersections( PolygonInTermsOfIntersections Expected_Unified_Polygon_Intersections ){
      return it_poly.ItIsAsExpected( Expected_Unified_Polygon_Intersections );
    }

bool PolygonInfo::CheckResultingPolygonInfoVectorOfEdgesForTestStructure( PolygonInfoVectorOfEdges pol_inf_vec_expected ){
      if( pol_inf_vec.size() != pol_inf_vec_expected.size() )
		  {
        std::cout<< "  incompatible size  pol_inf_vec.size()  " <<  pol_inf_vec.size() <<std::endl;
        std::cout<< "  incompatible size  pol_inf_vec_expected.size()  " <<  pol_inf_vec_expected.size() <<std::endl;
        return false;
      }  
      ...
      for( GridIndexType counter = 0 ; counter < pol_inf_vec.size() ; counter++ ){
        if( pol_inf_vec.at( counter ).size() != pol_inf_vec_expected.at( counter ).size() )
			{
          std::cout<< "  incompatible size  pol_inf_vec.at( counter ).size()  " <<  pol_inf_vec.at( counter ).size()  <<std::endl;
          std::cout<< "  incompatible size  pol_inf_vec_expected.at( counter ).size()  "  << pol_inf_vec_expected.at( counter ).size()  <<std::endl;
          return false;
        }  

        for( GridIndexType internal_counter = 0 ; internal_counter < pol_inf_vec.at(counter).size() ; internal_counter++ ){
    ...
    bool PolygonInfo::SameSnippet( PolygonSnippet& pol_snip_a , PolygonSnippet& pol_snip_b ){
      ...
      if( static_cast<float>( pol_snip_a.point_of_interest_start.x ) != static_cast<float>( pol_snip_b.point_of_interest_start.x ) )
		  {
		  return false;  
		  }
      if( static_cast<float>( pol_snip_a.point_of_interest_start.y ) != static_cast<float>( pol_snip_b.point_of_interest_start.y ) )
		  {
		  return false;  
		  }
      if( static_cast<float>( pol_snip_a.point_of_interest_end.y ) != static_cast<float>( pol_snip_b.point_of_interest_end.y ) )
		  {
		  return false;  
		  }
      ...
      if( pol_snip_a.snippet_intermediate_content.size() != pol_snip_b.snippet_intermediate_content.size() )
		  {
		  return false;  
		  }

      for( GridIndexType counter = 0 ; counter < pol_snip_a.snippet_intermediate_content.size() ; counter++ ){
        if(  static_cast<float>( pol_snip_a.snippet_intermediate_content.at(counter).x ) != static_cast<float>( pol_snip_b.snippet_intermediate_content.at(counter).x )  )
			{
		  return false;  
		  }
        if(  static_cast<float>( pol_snip_a.snippet_intermediate_content.at(counter).y ) != static_cast<float>( pol_snip_b.snippet_intermediate_content.at(counter).y )  )
			{
		  return false;  
		  }
        if(  static_cast<float>( pol_snip_a.snippet_intermediate_content.at(counter).z ) != static_cast<float>( pol_snip_b.snippet_intermediate_content.at(counter).z )  )
			{
		  return false;  
		  }
      }        
    ...
    bool PolygonInfo::CompareCalculatedAndExpectedResultingSequenceForTest( LegacyPolygonSequenceType& expected_resulting_sequence_ref ){
      if( expected_resulting_sequence_ref.size() > resulting_sequence.size()  )
		  {
        std::cout<<  " Size ! " <<std::endl;
        std::cout<<  " expected_resulting_sequence_ref.size() "  << expected_resulting_sequence_ref.size()  <<std::endl;
      ...
      for( GridIndexType counter = 0 ; counter < expected_resulting_sequence_ref.size() ; counter++ ){
        if( expected_resulting_sequence_ref.at( counter ).size() != resulting_sequence.at( counter ).size() )
          ...
          std::cout<<  " expected_resulting_sequence_ref.at( counter ).size() "  << expected_resulting_sequence_ref.at( counter ).size()  <<std::endl;
          std::cout<<  " resulting_sequence.at( counter ).size() "  <<  resulting_sequence.at( counter ).size() <<std::endl;
          return false;
        } 

        for( GridIndexType counter_internal = 0 ; counter_internal < expected_resulting_sequence_ref.at( counter ).size() ; counter_internal++ ){
    ...
    bool PolygonInfo::GetCornerFromVoxelIntersections( unsigned int the_index, SnippetIndex edge_index , IterativePolygonInTermsOfIntersections::WhichIntersection& intersect_index ){
      if( voxel_intersections.corner_intersections_intersection_coord.at( edge_index.edge ).size() > the_index )
		  {        
        intersect_index = voxel_intersections.corner_intersections_intersection_coord.at(edge_index.edge).at( the_index ); 
        return true;
      }
	  else{
		  return false;
    ...
    bool PolygonInfo::SetToNextIntersectonAlongThisEdgeAndReturnDistanceBasedOnVoxelIntersections( LegacyWorldCoordinate2D& closest_local_current_position_ref , int current_edge, VoxelIntersectionIndex next_voxel_intersection  ){
      if( voxel_intersections.edge_intersections_intersection_coord.at( current_edge ).size() > next_voxel_intersection.next_intersection )
		  {
        if( voxel_intersections.edge_intersections_intersection_coord.at( current_edge ).at( next_voxel_intersection.next_intersection ).size() > next_voxel_intersection.intersection_douplication )
			{        
          IterativePolygonInTermsOfIntersections::WhichIntersection an_intersection_index = voxel_intersections.edge_intersections_intersection_coord.at(current_edge).at(next_voxel_intersection.next_intersection).at( next_voxel_intersection.intersection_douplication ); 
    ...
    void PolygonInfo::CheckEdegeTouchCharacteristicsAndAppendIntermediateContent( SnippetIndex edge_index ){ 

          LegacyWorldCoordinate2D edge_coord =  pol_inf_vec.at( edge_index.edge ).at( edge_index.snip ).why_here;

          IterativePolygonInTermsOfIntersections::WhichIntersection the_intersection_index_from_voxel_intersections;
          ...
          IterativePolygonInTermsOfIntersections::WhichIntersection& the_intersection_index_from_voxel_intersections_ref = the_intersection_index_from_voxel_intersections;
          ...
          if( number_of_snippets_this_edge > 0 )
			  {
			  snip_vec_local = CreateSnippetVector( number_of_snippets_this_edge , pol_inf_vec.at( edge_index.edge ).at( edge_index.snip ) );
			  }
          int erased = 0;
      
          for( int which_one = 0 ; which_one < number_of_snippets_this_edge ; which_one++ ){     
            bool there_is_an_intersection = GetCornerFromVoxelIntersections( which_one, edge_index , the_intersection_index_from_voxel_intersections_ref );  

            if( there_is_an_intersection )
				{      
              it_poly.SetCurrentIntersectionIndex( the_intersection_index_from_voxel_intersections_ref );

              LegacyWorldCoordinate3D contour_point_one = it_poly.GetRestpectivePologonPointInVoxelCoordinates( the_intersection_index_from_voxel_intersections_ref );

              int back = 1; 

              while( (  contour_point_one.x == edge_coord.x ) && ( contour_point_one.y == edge_coord.y ) ){
                contour_point_one = it_poly.GetRestpectivePologonPointInVoxelCoordinatesPrevious( the_intersection_index_from_voxel_intersections_ref , back );     
              ...
              LegacyWorldCoordinate3D  contour_point_two = it_poly.GetRestpectivePologonPointInVoxelCoordinatesFurther( the_intersection_index_from_voxel_intersections_ref , ahead );      

              while( (  contour_point_two.x == edge_coord.x ) && ( contour_point_two.y == edge_coord.y ) ){
                ahead++;
         
                contour_point_two = it_poly.GetRestpectivePologonPointInVoxelCoordinatesFurther( the_intersection_index_from_voxel_intersections_ref , ahead );  
              }
              ...
              if( ( there_is_a_suitable_polygon_forward  ) && ( there_is_a_suitable_polygon_backward ) )
				  {             
                 WorkWithForwardAndBackwardEdgeSnippet( which_one, angle_charact_a , angle_charact_b , forward_snippet_ref , backward_snippet_ref , edge_snippet_ref , snip_vec_local , erased  );
              }
              if( ( there_is_a_suitable_polygon_forward  ) && ( ! there_is_a_suitable_polygon_backward ) )
				  {   
                WorkWithForwardEdgeSnippet( edge_coord, contour_point_one, which_one, angle_charact_a , angle_charact_b , forward_snippet_ref , backward_snippet_ref , edge_snippet_ref , snip_vec_local , erased  );
              }
              if( ( ! there_is_a_suitable_polygon_forward  ) && ( there_is_a_suitable_polygon_backward ) )
				  {
                WorkWithBackwardEdgeSnippet( edge_coord, contour_point_two , which_one, angle_charact_a , angle_charact_b , forward_snippet_ref , backward_snippet_ref , edge_snippet_ref , snip_vec_local , erased  );
              }    
          }
      }
    ...
    void PolygonInfo::WorkWithForwardAndBackwardEdgeSnippet( int which_one, float& angle_charact_a , float& angle_charact_b , PolygonSnippet& forward_snippet_ref , PolygonSnippet& backward_snippet_ref , PolygonSnippet& edge_snippet_ref , std::vector<PolygonSnippet>& snip_vec_local , int& erased  ){
    ...
    void PolygonInfo::WorkWithForwardEdgeSnippet( LegacyWorldCoordinate2D edge_coord, LegacyWorldCoordinate3D contour_point_one, int which_one, float& angle_charact_a , float& angle_charact_b , PolygonSnippet& forward_snippet_ref , PolygonSnippet& backward_snippet_ref , PolygonSnippet& edge_snippet_ref , std::vector<PolygonSnippet>& snip_vec_local , int& erased  ){
    ...
    PolygonInfo::snippet_vector PolygonInfo::CreateSnippetVector( int number_of_snippets_this_edge , PolygonSnippet template_snip ){
...
do_check.clear();
    ...
    void PolygonInfo::InsertToSnippetVectorFront( snippet_vector snip_vec_local ,  SnippetIndex edge_index ){
      snippet_vector_iterator iter;
    ...
    void PolygonInfo::RemoveFromLocalSnippetVector( snippet_vector& snip_vec_local , GridIndexType which_one ){
      snippet_vector_iterator iter;
    ...
    void PolygonInfo::GetSectorsAndSetCharacteristicsDoubleCheck( SnippetIndex edge_index, LegacyWorldCoordinate2D edge_coord , LegacyWorldCoordinate3D& contour_point_one , LegacyWorldCoordinate3D& contour_point_two ){
      ...
      GetSectors( edge_coord , contour_point_one , contour_point_two , sector_first_point_ref ,  sector_second_point_ref );                 

      if( edge_index.edge  == 0 )
		  {
        CharacterizeIntersection charact_double_check = GetCharacteristicFirstCorner(  sector_first_point_ref , sector_second_point_ref );
        if( ( a_snippet_ref.characterize != charact_double_check ) && ( a_snippet_ref.characterize != corner  )  )
			{
          std::cout<< " Have double checked the characteristic of corner zero. It turned out not to be determined correctly ! " <<std::endl;
          assert( 0 ); 
        }
      } 
      else if( edge_index.edge  == 1 )
		  {
        CharacterizeIntersection charact_double_check = GetCharacteristicSecondCorner(  sector_first_point_ref , sector_second_point_ref );
        if(  ( a_snippet_ref.characterize != charact_double_check )&& ( a_snippet_ref.characterize != corner  )  )
			{
          std::cout<< " Have double checked the characteristic of corner one. It turned out not to be determined correctly ! " <<std::endl;
          assert( 0 );
        }
      }
      else if( edge_index.edge  == 2 )
		  {
        CharacterizeIntersection charact_double_check = GetCharacteristicThirdCorner(  sector_first_point_ref , sector_second_point_ref );
        if( ( a_snippet_ref.characterize != charact_double_check ) && ( a_snippet_ref.characterize != corner  ) )
			{
          std::cout<< " Have double checked the characteristic of corner two. It turned out not to be determined correctly ! " <<std::endl;
          assert( 0 ); 
        }
      }
      else if( edge_index.edge  == 3 )
		  {
        CharacterizeIntersection charact_double_check = GetCharacteristicFourthCorner(  sector_first_point_ref , sector_second_point_ref );
        if( ( a_snippet_ref.characterize != charact_double_check ) && ( a_snippet_ref.characterize != corner  ) )
			{
          std::cout<< " Have double checked the characteristic of corner three. It turned out not to be determined correctly ! " <<std::endl;
          assert( 0 ); 
        }
      }
	  else{
    ...
    void PolygonInfo::GetSectors( LegacyWorldCoordinate2D edge_coord , LegacyWorldCoordinate3D& contour_point_one , LegacyWorldCoordinate3D& contour_point_two, WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref ){
    ...
    void PolygonInfo::GetSectorFirstPoint( LegacyWorldCoordinate2D edge_coord , LegacyWorldCoordinate3D& contour_point_one , WhichSector& sector_first_point_ref ){
      if( ( contour_point_one.x == edge_coord.x ) && ( contour_point_one.y < edge_coord.y ) )
		  {
	...
	void PolygonInfo::GetSectorSecondPoint( LegacyWorldCoordinate2D edge_coord , LegacyWorldCoordinate3D& contour_point_two, WhichSector& sector_second_point_ref ){
      if( ( contour_point_two.x == edge_coord.x ) && ( contour_point_two.y < edge_coord.y ) )
		  {
    ...
    CharacterizeIntersection PolygonInfo::GetCharacteristicFirstCorner(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref  ){
      ...
      DoCharacterize(  sector_first_point_ref ,  sector_second_point_ref , charact_ref , in_combination_vector ,  inside_ref , preceding_edge_ref , following_edge_ref );
    ...
    void PolygonInfo::DoCharacterize( WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , CharacterizeIntersection& charact_ref , std::vector<WhichSector> in_combination_vector , WhichSector& inside_ref , WhichSector& preceding_edge_ref , WhichSector& following_edge_ref ){
      if( BothOnEdge(  sector_first_point_ref ,  sector_second_point_ref , preceding_edge_ref , following_edge_ref ) )
		  {
		  charact_ref = edge_to_edge;}
      else if( FromEdgeIn(  sector_first_point_ref ,  sector_second_point_ref , preceding_edge_ref , inside_ref ) ) 
		  {
		  charact_ref = edge_to_inside;
		  }
      else if( FromEdgeOut(  sector_first_point_ref ,  sector_second_point_ref , preceding_edge_ref , in_combination_vector ) )
		  {
		  charact_ref = edge_to_outside;
		  }
      else if( OutToEdge(  sector_first_point_ref ,  sector_second_point_ref , following_edge_ref , in_combination_vector ) )
		  {
		  charact_ref = outside_to_edge;
		  }
      else if( InToEdge(  sector_first_point_ref ,  sector_second_point_ref ,  following_edge_ref , inside_ref ) ) 
		  {
      ...
      else if( OneInOneOut(  sector_first_point_ref ,  sector_second_point_ref , inside_ref , in_combination_vector ) ) charact_ref = real_intersection;
      else assert( 0 );
    }   
    ...
    bool PolygonInfo::BothOnEdge(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& preceding_edge_ref , WhichSector& following_edge_ref ){
      ...
      bool it_is_a = FirstSectorIsOneOfTheseAndSecondSectorIsOneOfThose(  sector_first_point_ref ,  first_sector_vector  , sector_second_point_ref , second_sector_vector );
      bool it_is_b = FirstSectorIsOneOfTheseAndSecondSectorIsOneOfThose(  sector_second_point_ref ,  first_sector_vector  , sector_first_point_ref , second_sector_vector  );

      if( it_is_a || it_is_b ) return true;
      else return false;

    }


    bool PolygonInfo::FromEdgeIn(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& preceding_edge_ref , WhichSector& inside_ref ){
    ...
    bool PolygonInfo::FromEdgeOut(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& preceding_edge_ref ,  std::vector<WhichSector> second_sector_vector  ){
      ...
      bool it_is_a = FirstSectorIsOneOfTheseAndSecondSectorIsNoneOfThose(  sector_first_point_ref ,  first_sector_vector  , sector_second_point_ref , second_sector_vector );
      bool it_is_b = FirstSectorIsOneOfTheseAndSecondSectorIsNoneOfThose(  sector_second_point_ref ,  first_sector_vector  , sector_first_point_ref , second_sector_vector );
    ...
    bool PolygonInfo::OutToEdge(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& following_edge_ref ,  std::vector<WhichSector> second_sector_vector  ){
    ...
    bool PolygonInfo::InToEdge(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& following_edge_ref , WhichSector& inside_ref ){
    ...
    bool PolygonInfo::BothIn(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& inside_ref ){

      std::vector<WhichSector> sector_vector;
      WhichSector sec_inside = inside_ref;
      sector_vector.push_back( sec_inside );

      bool it_is_a = FirstSectorIsOneOfTheseAndSecondSectorIsOneOfThose(  sector_first_point_ref ,  sector_vector  , sector_second_point_ref , sector_vector );

      if( it_is_a ) return true;
      else return false;

    }


    bool PolygonInfo::BothOut(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , std::vector<WhichSector> sector_vector ){
      ...
      bool it_is_a = FirstSectorIsNoneOfTheseAndSecondSectorIsNoneOfThose(  sector_first_point_ref ,  sector_vector  , sector_second_point_ref , sector_vector );
    ...
    bool PolygonInfo::OneInOneOut(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& inside_ref , std::vector<WhichSector> first_sector_vector ){
      ...
      bool it_is_a = FirstSectorIsOneOfTheseAndSecondSectorIsNoneOfThose(  sector_first_point_ref  , second_sector_vector  , sector_second_point_ref ,  first_sector_vector );
      bool it_is_b = FirstSectorIsOneOfTheseAndSecondSectorIsNoneOfThose(  sector_second_point_ref , second_sector_vector  , sector_first_point_ref  ,  first_sector_vector );
    ...
    bool PolygonInfo::FirstSectorIsOneOfTheseAndSecondSectorIsOneOfThose(  WhichSector& sector_first_point_ref , std::vector<WhichSector> first_sector_vector , WhichSector& sector_second_point_ref , std::vector<WhichSector> second_sector_vector ){

      bool first_one_of_these = false;
      bool second_one_of_those = false;

      for( unsigned int index = 0 ; index < first_sector_vector.size() ; index++ ){
        if( first_sector_vector.at( index ) == sector_first_point_ref ) first_one_of_these = true;
      }

      for( unsigned int index = 0 ; index < second_sector_vector.size() ; index++ ){
        if( second_sector_vector.at( index ) == sector_second_point_ref ) second_one_of_those = true;
      }

      if( first_one_of_these && second_one_of_those )return true;
      else return false;
    ...
    bool PolygonInfo::FirstSectorIsOneOfTheseAndSecondSectorIsNoneOfThose(  WhichSector& sector_first_point_ref , std::vector<WhichSector> first_sector_vector , WhichSector& sector_second_point_ref , std::vector<WhichSector> second_sector_vector ){
      ...
      for( unsigned int index = 0 ; index < second_sector_vector.size() ; index++ ){
        if( second_sector_vector.at( index ) == sector_second_point_ref ) second_none_of_those = false;
      }

      if( first_one_of_these && second_none_of_those )return true;
      else return false;
    ...
    bool PolygonInfo::FirstSectorIsNoneOfTheseAndSecondSectorIsNoneOfThose(  WhichSector& sector_first_point_ref , std::vector<WhichSector> first_sector_vector , WhichSector& sector_second_point_ref , std::vector<WhichSector> second_sector_vector ){

      bool first_none_of_these = true;
      bool second_none_of_those = true;

      for( unsigned int index = 0 ; index < first_sector_vector.size() ; index++ ){
        if( first_sector_vector.at( index ) == sector_first_point_ref ) first_none_of_these = false;
      }
      ...
      if( first_none_of_these && second_none_of_those )return true;
      else return false;
    ...
    CharacterizeIntersection PolygonInfo::GetCharacteristicSecondCorner( WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref  ){
    ...
    CharacterizeIntersection PolygonInfo::GetCharacteristicThirdCorner(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref  ){
    ...
    CharacterizeIntersection PolygonInfo::GetCharacteristicFourthCorner( WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref  ){
    ...
    bool PolygonInfo::AppendNextIntersectionAndCorrespoindingPolygonSnippets( snippet_vector& snip_vec_ref_local , snippet_vector& snip_vec_ref , int current_edge, VoxelIntersectionIndex& next_voxel_intersection ){
      ...
      while( im_done == false ){
        ...
        got_one = SetToNextIntersectonAlongThisEdgeAndReturnDistanceBasedOnVoxelIntersections( closest_local_current_position_ref, current_edge, next_voxel_intersection );
        ...
        if( changed_loc ){
          ...
          if( snip_vec_ref_local.size() > 1 ) snip_vec_ref_local = SortClockwise( snip_vec_ref_local ); 
    ...
    void PolygonInfo::IncrementVoxelIntersectionIndex( int current_edge, VoxelIntersectionIndex& vox_inters_index , bool& changed_location ){

      if( voxel_intersections.edge_intersections_intersection_coord.at( current_edge ).size() > vox_inters_index.next_intersection ){
   
        if( voxel_intersections.edge_intersections_intersection_coord.at( current_edge ).at( vox_inters_index.next_intersection ).size() > ( vox_inters_index.intersection_douplication + 1 ) ){
    ...
    float PolygonInfo::SetToNextIntersectionAlongThisEdgeAndReturnDistance( LegacyWorldCoordinate2D& closest_local_current_position_ref , float distance_current_position , int current_edge ){
     ...
     // if( it_poly.CheckToBeRegarded() ){ 
        ...
        if( it_poly.ThisIntersection( local_current_position ) ){ 
          ...
          if( IsTheSameEdgeButNoCorner( local_current_position , current_position , current_edge ) ){
            ...
            if( ( static_cast<float>( distance_current_position_local ) > static_cast<float>( distance_current_position ) ) && ( static_cast<float>( distance_next_edge ) > static_cast<float>( distance_current_position_local ) ) ){ // betrachtet werden nur die Schnittpunkte, die im Uhrzeigersinn 
    ...
    bool PolygonInfo::IsTheSameEdgeButNoCorner( LegacyWorldCoordinate2D local_current_position , LegacyWorldCoordinate2D current_position , int current_edge ){
      ...
      switch( current_edge ){
       ...
       case 0 : 
         ...
         if( ( local_current_position.y == current_position.y ) && ( local_current_position.x <= ( the_dose_index.x + 1 ) ) && ( local_current_position.x >= the_dose_index.x ) && ( the_dose_index.y == local_current_position.y ) ){

           if(  ( static_cast<float>( corner_coord.x ) == static_cast<float>( local_current_position.x ) ) && (  static_cast<float>( corner_coord.y ) == static_cast<float>( local_current_position.y )  )  ) return false;   
           corner_coord.x += 1;
           if(  ( static_cast<float>( corner_coord.x ) == static_cast<float>( local_current_position.x ) ) && (  static_cast<float>( corner_coord.y ) == static_cast<float>( local_current_position.y )  )  ) return false;                             
           return true;
       ...
       case 1 : 
         ...
         if( ( local_current_position.x == current_position.x ) && ( local_current_position.y <= ( the_dose_index.y + 1 ) ) && ( local_current_position.y >= the_dose_index.y ) && ( ( the_dose_index.x + 1 ) == local_current_position.x ) ){
           corner_coord.x += 1;
           if(  ( static_cast<float>( corner_coord.x ) == static_cast<float>( local_current_position.x ) ) && (  static_cast<float>( corner_coord.y ) == static_cast<float>( local_current_position.y )  )  ) return false;   
           corner_coord.y += 1;
           if(  ( static_cast<float>( corner_coord.x ) == static_cast<float>( local_current_position.x ) ) && (  static_cast<float>( corner_coord.y ) == static_cast<float>( local_current_position.y )  )  ) return false;                             
       ...
       case 2 : 
         ...
         if( ( local_current_position.y == current_position.y ) && ( local_current_position.x <= ( the_dose_index.x + 1 ) ) && ( local_current_position.x >= the_dose_index.x ) && ( ( the_dose_index.y + 1 ) == local_current_position.y ) ){
           corner_coord.x += 1;
           corner_coord.y += 1; 
           if(  ( static_cast<float>( corner_coord.x ) == static_cast<float>( local_current_position.x ) ) && (  static_cast<float>( corner_coord.y ) == static_cast<float>( local_current_position.y )  )  ) return false;   
           corner_coord.x -= 1;
       ...
       case 3 :                      
         ...
         if( ( local_current_position.x == current_position.x ) && ( local_current_position.y <= ( the_dose_index.y + 1 ) ) && ( local_current_position.y >= the_dose_index.y ) && ( the_dose_index.x == local_current_position.x ) ){
           corner_coord.y += 1; 
           if(  ( static_cast<float>( corner_coord.x ) == static_cast<float>( local_current_position.x ) ) && (  static_cast<float>( corner_coord.y ) == static_cast<float>( local_current_position.y )  )  ) return false;   
           corner_coord.y -= 1;
           if(  ( static_cast<float>( corner_coord.x ) == static_cast<float>( local_current_position.x ) ) && (  static_cast<float>( corner_coord.y ) == static_cast<float>( local_current_position.y )  )  ) return false;                             
    ...
    bool PolygonInfo::AddSnippetsThisIntersection( snippet_vector& snip_vec_ref , LegacyWorldCoordinate2D closest_local_current_position ){
      ...
      if(   ( there_is_a_suitable_polygon_forward  ) || ( there_is_a_suitable_polygon_backward )   ) return true;
      else return false;
    ...
    bool PolygonInfo::JustEdge( PolygonSnippet& snippet_ref ){
      ...
      if(  ( current_edge == 0 )  ||  ( current_edge == 2 )  ){      
        for( unsigned int position = 0 ; position < snippet_ref.snippet_intermediate_content.size() ; position++ ){
          LegacyWorldCoordinate3D w_coord = snippet_ref.snippet_intermediate_content.at( position );
          if( static_cast<float>( w_coord.y ) != static_cast<float>( snippet_ref.point_of_interest_start.y ) ) just_edge = false;
        }
        if( static_cast<float>( snippet_ref.point_of_interest_start.y ) != static_cast<float>( snippet_ref.point_of_interest_end.y ) ) just_edge = false; 
      }
      else if(  ( current_edge == 1 ) || ( current_edge == 3 )  ){
        if( static_cast<float>( snippet_ref.point_of_interest_start.x ) != static_cast<float>( snippet_ref.point_of_interest_end.x ) ) just_edge = false;      
        for( unsigned int position = 0 ; position < snippet_ref.snippet_intermediate_content.size() ; position++ ){
          LegacyWorldCoordinate3D w_coord = snippet_ref.snippet_intermediate_content.at( position );
          if( static_cast<float>( w_coord.x ) != static_cast<float>( snippet_ref.point_of_interest_start.x ) ) just_edge = false;
        }
      }  
      else assert( 0 ); 
    ...
    bool PolygonInfo::GoForwardAndCreatePolygonSnippet( PolygonSnippet& forward_snippet_ref ){
      ...
      if(  is_inside_valid  ){  
        ...
        std::vector<LegacyWorldCoordinate3D>& snippet_intermediate_content_ref = forward_snippet_ref.snippet_intermediate_content;
        it_poly.RunForwardToNextIntersection(  point_of_interest_start_ref , point_of_interest_end_ref , snippet_intermediate_content_ref , the_dose_index );  
    ...
    bool PolygonInfo::GoBackwardAndCreatePolygonSnippet( PolygonSnippet& backward_snippet_ref ){
      ...
      if(  is_inside_valid  ){  
        ...
        std::vector<LegacyWorldCoordinate3D>& snippet_intermediate_content_ref = backward_snippet_ref.snippet_intermediate_content;

        it_poly.RunBackwardToNextIntersection(  point_of_interest_start_ref , point_of_interest_end_ref , snippet_intermediate_content_ref  );  
    ...
    bool PolygonInfo::CheckPolygonIsInsideForward( float& angle_charact_forward  ){
      ...
      if( it_poly.ThisIntersection( the_first_point_ref ) ){
        ...
        bool got_it = it_poly.NextPointOrIntersectionPeriodically( the_first_point_ref, the_next_point_ref, the_dose_index );
        ...
        while( (  stop_counter < ( maxNum + 2 )  ) ){   
          ...
          */
          ...
          if( ( static_cast<float>( the_next_point.x ) != static_cast<float>( the_first_point.x ) ) || ( static_cast<float>( the_next_point.x ) != static_cast<float>( the_first_point.x ) ) ) stop_counter = ( maxNum + 2 ); 
          if( ( static_cast<float>( the_next_point.y ) != static_cast<float>( the_first_point.y ) ) || ( static_cast<float>( the_next_point.y ) != static_cast<float>( the_first_point.y ) ) ) stop_counter = ( maxNum + 2 );          

          if( stop_counter < ( maxNum + 2 ) ) got_it = it_poly.NextPointOrIntersectionPeriodically( the_first_point_ref, the_next_point_ref, the_dose_index );

        }

        if( got_it == false )assert( 0 );


        if(  ( got_it ) &&  (  ( static_cast<float>( the_next_point.x ) != static_cast<float>( the_first_point.x ) ) || ( static_cast<float>( the_next_point.y ) != static_cast<float>( the_first_point.y ) )  )  ){
          ...
          if( CheckPointInVoxelRegardingDoseIndex( an_intermediate_point ) )  its_inside = true;

        }
        else  its_inside = false;
    ...
    void PolygonInfo::GetAngle( LegacyWorldCoordinate2D the_next_point , LegacyWorldCoordinate2D the_first_point , float& angle_charact ){

      double hypoten = ( the_next_point.x - the_first_point.x ) * ( the_next_point.x - the_first_point.x )  +  ( the_next_point.y - the_first_point.y )  *  ( the_next_point.y - the_first_point.y );

      assert( hypoten > 0 );  // This should never happen except in case of a structure consisting of one point only. However, that's not allowed in voxelisation.
    ...
    bool PolygonInfo::CheckPolygonIsInsideBackward( float& angle_charact_backward ){
      ...
      if( it_poly.ThisIntersection( the_first_point_ref ) ){
        ...
        bool got_it = it_poly.PreviousPointOrIntersectionPeriodically( the_first_point_ref , the_next_point_ref , the_dose_index );
        ...
        while( ( stop_counter < maxNum  )  ){   
          ...
          if( ( static_cast<float>( the_next_point.x ) != static_cast<float>( the_first_point.x ) ) || ( static_cast<float>( the_next_point.x ) != static_cast<float>( the_first_point.x ) ) )  stop_counter = maxNum;

          if( ( static_cast<float>( the_next_point.y ) != static_cast<float>( the_first_point.y ) ) || ( static_cast<float>( the_next_point.y ) != static_cast<float>( the_first_point.y ) ) )  stop_counter = maxNum; 



          if( stop_counter < maxNum ) got_it = it_poly.PreviousPointOrIntersectionPeriodically( the_first_point_ref , the_next_point_ref , the_dose_index );
    ...
    bool PolygonInfo::CheckPointInVoxelRegardingDoseIndex( LegacyWorldCoordinate2D voxel_coordinate_to_be_checked ){

      bool its_inside = false;

      if(   ( voxel_coordinate_to_be_checked.x  <=  ( the_dose_index.x + 1 )  )   &&   ( voxel_coordinate_to_be_checked.x   >= the_dose_index.x  )   &&    ( voxel_coordinate_to_be_checked.y   <=  ( the_dose_index.y + 1 )  ) && ( voxel_coordinate_to_be_checked.y   >=  the_dose_index.y   )  )  its_inside = true;   
    ...
    float PolygonInfo::GetDistanceAlongEdge( LegacyDoseVoxelIndex3D edge_position , LegacyWorldCoordinate2D local_position  ){
      ...
      if( local_position.y == edge_position.y ){
        ...
        return_distance = GInformation.getPixelSpacingRow() - ( local_position.x - edge_position.x ) * GInformation.getPixelSpacingRow(); 
        return_distance += GInformation.getPixelSpacingColumn();
        return_distance += GInformation.getPixelSpacingRow();
        ...
        return_distance = 2 * GInformation.getPixelSpacingRow();
        return_distance += GInformation.getPixelSpacingColumn();
        return_distance += GInformation.getPixelSpacingColumn() - ( local_position.y - edge_position.y ) * GInformation.getPixelSpacingColumn();  
    ...
    float PolygonInfo::GetPeriodicDist( float distance_current_position_local , float distance_current_position ){

      float distance_oneD_return;

      float max_distance = static_cast<float>( 2 * GInformation.getPixelSpacingRow() + 2 * GInformation.getPixelSpacingColumn() );
      ...
      if( distance_current_position_local >= distance_current_position ){
        ...
        float cross_edge_distance = ( max_distance - distance_current_position_local ) + distance_current_position;

        if( cross_edge_distance < distance_oneD_return ) distance_oneD_return = cross_edge_distance;
        ...
        float cross_edge_distance = ( max_distance - distance_current_position ) + distance_current_position_local;
    ...
    double PolygonInfo::GetClockwiseDist( double distance_refer_to , double distance_compare ){
      ...
      double max_distance = static_cast<double>( 2 * GInformation.getPixelSpacingRow() + 2 * GInformation.getPixelSpacingColumn() );
    ...
    */  
    ...
    bool PolygonInfo::CreatePolygonSequence(){
      ...
      bool its_inside = SetCurrentStaticIndexKnownInsideOrOutside( current_static_index_ref , nothing_to_do_ref );  // Falls es eine Ecke gibt, die draussen liegt wird hier false zurueckgegeben.
      bool just_switched_inside = false;
      if( its_inside ) just_switched_inside = true;
  ...
  /*    std::cout<<  "  the_dose_index.x  : "  <<  the_dose_index.x  <<std::endl; 
      ...
      if( nothing_to_do == false ){ 
        ...
        while( ( ! SnippetIndexIdentical( begin_index , current_static_index ) ) || ( its_first_current_static == true ) ){
          ...
          if( ( ( ( its_inside ) && ( just_switched_inside_ref ) ) || ( next_is_edge == false ) ) && ( ! CurrentSnippetIsProcessed() ) ){
...
//if(  ( the_dose_index.x == 3 ) && ( the_dose_index.y == 2 )  )  ShowResultingSequence();
    ...
    void PolygonInfo::SlicePositionUnifyAndCheckPeriodicSequence(){
      ...
      for( unsigned int first_index = 0 ; first_index < resulting_sequence.size() ; first_index++ ){
        ...
        for(  unsigned int second_index = 0 ; second_index < resulting_sequence.at( first_index ).size() ; second_index++  ){


          LegacyWorldCoordinate3D the_world_coordinate_3D = resulting_sequence.at( first_index ).at( second_index );
        ...
        for(  unsigned int second_index = 0 ; second_index < resulting_sequence.at( first_index ).size() ; second_index++  ){
          ...
          LegacyWorldCoordinate3D& the_world_coordinate_3D_ref = resulting_sequence.at( first_index ).at( second_index );
        ...
        if( resulting_sequence.at( first_index ).size() > 0  ){
          if( static_cast<float>( resulting_sequence.at( first_index ).at( resulting_sequence.at( first_index ).size() - 1 ).x ) !=  static_cast<float>( resulting_sequence.at( first_index ).at(0).x ) )assert(0); // this must never happen. The countour must be of periodic nature. 
          if( static_cast<float>( resulting_sequence.at( first_index ).at( resulting_sequence.at( first_index ).size() - 1 ).y ) !=  static_cast<float>( resulting_sequence.at( first_index ).at(0).y ) )assert(0); // this must never happen. The countour must be of periodic nature. 
          if( static_cast<float>( resulting_sequence.at( first_index ).at( resulting_sequence.at( first_index ).size() - 1 ).z ) !=  static_cast<float>( resulting_sequence.at( first_index ).at(0).z ) )assert(0); // this must never happen. The countour must be of periodic nature. 
        }
    ...
    void PolygonInfo::GoGetPoly( bool& do_continue_ref , bool& next_is_edge_ref, int& continue_counter_ref , int& max_num_ref ){
      ...
      if( next_is_edge_ref ){
        ...
        if( do_continue_ref ) do_continue_ref = DoContinue( max_num_ref , continue_counter_ref ); 
        if( do_continue_ref == false )interupt = true;           
        while( ! interupt ){

          if(  TryGetPolygonsnippetEdge( continue_counter_ref , the_previous_edge ) ){  // holt sich das Schnipsel, das zum moving index gehoert falls es Konturinhalt hat. Falls nicht wird der moving index weiter gesetzt.
            interupt = true;
        ...
        DoAppendIntermediateEdgeUpToThisSnippet( snipp_ref , why_here , the_previous_edge , current_moving_index.edge );
        ...
        else assert(0); //this should never happen     
    ...
    bool PolygonInfo::CurrentSnippetIsProcessed(){     
      return pol_inf_vec.at( current_static_index.edge ).at( current_static_index.snip ).i_have_been_processed;
    }
    ...
    void PolygonInfo::ShowResultingSequence(){
      ...
      for( GridIndexType counter = 0 ; counter < resulting_sequence.size() ; counter++ ){
        ...
        for( GridIndexType counter_internal = 0 ; counter_internal < resulting_sequence.at( counter ).size() ; counter_internal++ ){
    ...
    bool PolygonInfo::GetCurrentMovingSnippet( PolygonSnippet& snipp_ref ){
      ...
      if(  ( current_moving_index.edge < 4 ) && ( current_moving_index.snip < pol_inf_vec.at( current_moving_index.edge ).size() ) && ( current_moving_index.snip >= 0 ) && ( current_moving_index.edge >= 0 )  ){
    ...
    void PolygonInfo::SetCurrentMovingProcessed(){
      if(  ( current_moving_index.edge < 4 ) && ( current_moving_index.snip < pol_inf_vec.at( current_moving_index.edge ).size() ) && ( current_moving_index.snip >= 0 ) && ( current_moving_index.edge >= 0 )  ){
        pol_inf_vec.at( current_moving_index.edge ).at( current_moving_index.snip ).i_have_been_processed = true;   
      }
    }
    ...
    void PolygonInfo::IncrementEdge( int& edge_increment ){
      ...
      if( edge_increment == 3 ) edge_increment = 0;
      else edge_increment++; 

    }


    void PolygonInfo::DoAppendIntermediateEdgeUpToThisSnippet( PolygonSnippet& snipp_ref , LegacyWorldCoordinate2D why_here , int the_previous_edge , int the_current_edge ){
      ...
      std::vector<LegacyWorldCoordinate3D>& inter_content_ref = snipp_internal.snippet_intermediate_content;

      int edge_internal = the_previous_edge;

      while( EdgeDist( edge_internal , the_current_edge ) > 0 ){
        edge_internal++;
        if( edge_internal == 4 ) edge_internal = 0;
        AppendEdgeToVector( inter_content_ref , edge_internal );
      } 

      if( snipp_internal_ref.snippet_intermediate_content.size() > 0 ){
        if( ( snipp_internal_ref.snippet_intermediate_content.at( 0 ).x == snipp_internal_ref.point_of_interest_end.x ) && ( snipp_internal_ref.snippet_intermediate_content.at( 0 ).y == snipp_internal_ref.point_of_interest_end.y ) ) snipp_internal_ref.snippet_intermediate_content.clear();
      }

      if( snipp_internal_ref.snippet_intermediate_content.size() > 0 ){
        std::vector<LegacyWorldCoordinate3D>::iterator iter;
        while( ( snipp_internal_ref.snippet_intermediate_content.at( snipp_internal_ref.snippet_intermediate_content.size() - 1 ).x == snipp_internal_ref.point_of_interest_end.x ) && ( snipp_internal_ref.snippet_intermediate_content.at( snipp_internal_ref.snippet_intermediate_content.size() - 1 ).y == snipp_internal_ref.point_of_interest_end.y ) ){
    ...
    void PolygonInfo::AppendEdgeToVector( std::vector<LegacyWorldCoordinate3D>& vector , int edge_internal ){
    ...
    void PolygonInfo::AppendToResultingSequence(  PolygonSnippet& snipp_ref  ){
      ...
      if( resulting_sequence.at( which_polygon ).size() > 0 ){
        ...
        LegacyWorldCoordinate3D the_world_coordinate_3D = resulting_sequence.at( which_polygon ).at( resulting_sequence.at( which_polygon ).size() - 1 );

        if( ( snipp_ref.point_of_interest_start.x == the_world_coordinate_3D.x ) && ( snipp_ref.point_of_interest_start.y == the_world_coordinate_3D.y ) ){ 

          DoAppendForward( snipp_ref ,  which_polygon );   

        }
        else if( ( snipp_ref.point_of_interest_end.x == the_world_coordinate_3D.x ) && ( snipp_ref.point_of_interest_end.y == the_world_coordinate_3D.y ) ){
      ...
      else AddFirstForward( snipp_ref ,  which_polygon );
    ...
    void PolygonInfo::DoInsertForward( PolygonSnippet& snipp_ref , int which_polygon ){
      ...
      for( unsigned int counter = 0 ; counter < snipp_ref.snippet_intermediate_content.size() ; counter++ ){
        ...
        resulting_sequence.at( which_polygon ).insert( it , snipp_ref.snippet_intermediate_content.at( counter ) ); 
      }
    ...
    void PolygonInfo::GoAlongSnippetToNextIntersectionAndAppendToResultingSequence( PolygonSnippet& snipp_ref ,  int& continue_counter_ref ){
    ...
    bool PolygonInfo::IsCorresponding(  PolygonSnippet& snipp_ref_compare , PolygonSnippet& snipp_ref ){
      ...
      if(  ( static_cast<float>( snipp_ref_compare.why_here.x ) == static_cast<float>( snipp_ref.point_of_interest_end.x ) ) && ( static_cast<float>( snipp_ref_compare.why_here.y ) == static_cast<float>( snipp_ref.point_of_interest_end.y )  )  ){

        if(  ! (  ( static_cast<float>( snipp_ref_compare.point_of_interest_end.x ) == static_cast<float>( snipp_ref.why_here.x ) )  || (  static_cast<float>( snipp_ref_compare.point_of_interest_start.x ) == static_cast<float>( snipp_ref.why_here.x )  )  ) && ( ( static_cast<float>( snipp_ref_compare.point_of_interest_end.y ) == static_cast<float>( snipp_ref.why_here.y )  ) ||  (  static_cast<float>( snipp_ref_compare.point_of_interest_start.y ) == static_cast<float>( snipp_ref.why_here.y )  )   )  ) return false;

        if( snipp_ref_compare.snippet_intermediate_content.size() != snipp_ref.snippet_intermediate_content.size() ) return false;

        for( GridIndexType counter = 0 ; counter < snipp_ref_compare.snippet_intermediate_content.size() ; counter++ ){

          if( static_cast<float>( snipp_ref_compare.snippet_intermediate_content.at( counter ).x ) != static_cast<float>( snipp_ref.snippet_intermediate_content.at( snipp_ref.snippet_intermediate_content.size() - ( counter + 1 )  ).x  ) ) return false;
          if( static_cast<float>( snipp_ref_compare.snippet_intermediate_content.at( counter ).y ) != static_cast<float>( snipp_ref.snippet_intermediate_content.at( snipp_ref.snippet_intermediate_content.size() - ( counter + 1 )  ).y  ) ) return false;
    ...
    LegacyWorldCoordinate2D PolygonInfo::GetOtherEnd( PolygonSnippet& snipp_ref ){
      ...
      if( ( snipp_ref.why_here.x == snipp_ref.point_of_interest_start.x ) && ( snipp_ref.why_here.y == snipp_ref.point_of_interest_start.y ) ){
        return snipp_ref.point_of_interest_end;
      } 
      else if(  ( snipp_ref.why_here.x == snipp_ref.point_of_interest_end.x ) && ( snipp_ref.why_here.y == snipp_ref.point_of_interest_end.y )  ){
    ...
    int PolygonInfo::GetSnippetIndexDistance( SnippetIndex behind_snip_index , SnippetIndex front_snip_index ){
    ...
    bool PolygonInfo::SnippetIndexIdentical( SnippetIndex first , SnippetIndex second ){
      if( (  first.edge  ==  second.edge  ) && (  first.snip  ==  second.snip  ) )return true;
      else return false;
    }



    void PolygonInfo::FindCorrespondingOneAndSetMovingIndexAndSetContinueCounter(  PolygonSnippet& snipp_ref , int& continue_counter_ref ){
      ...
      while(  ( !there_is_one  ) &&  (  ( another_snip_index.edge != current_moving_index.edge ) || ( another_snip_index.snip != current_moving_index.snip )  )  ){
      
            PolygonSnippet& snipp_ref_compare = pol_inf_vec.at( another_snip_index.edge ).at( another_snip_index.snip ) ;
    ...
    bool PolygonInfo::IsOtherEdge(  PolygonSnippet& snipp_ref  , int the_previous_edge  ){
      ...
      if( ( the_previous_edge == 0 ) || ( the_previous_edge == 2 )  ){
        ...
        if( static_cast<float>( snipp_ref.point_of_interest_start.y )  !=  static_cast<float>( snipp_ref.point_of_interest_end.y ) ) return true;

      }
      else if( ( the_previous_edge == 1 ) || ( the_previous_edge == 3 ) ){

        if( static_cast<float>( snipp_ref.point_of_interest_start.x )  != static_cast<float>(  snipp_ref.point_of_interest_end.x ) ) return true;
    ...
    bool PolygonInfo::TryGetPolygonsnippetEdge(  int& continue_counter_ref , int the_previous_edge ){
      ...
      if( pol_inf_vec.at( current_moving_index.edge ).at( current_moving_index.snip ).snippet_intermediate_content.size() > 0 ){
      ...
      else if( IsOtherEdge( pol_inf_vec.at( current_moving_index.edge ).at( current_moving_index.snip ) , the_previous_edge  )  ){
    ...
    void PolygonInfo::IncrementSnippetIndex( SnippetIndex& the_snippet_index ){
      ...
      if( the_snippet_index.snip >= pol_inf_vec.at( the_snippet_index.edge ).size() ){
        ...
        if( the_snippet_index.edge > 3 ) the_snippet_index.edge = 0;                 
    ...
    bool PolygonInfo::GetNextIsEdge( SnippetIndex  snip_index ){
      ...
      CharacterizeIntersection characterize = pol_inf_vec.at( snip_index.edge ).at( snip_index.snip ).characterize;
    ...
    bool PolygonInfo::DoContinue( int& max_num , int& continue_counter_ref ){  
      ...
      if( max_num > ( continue_counter_ref ) ) return true;
      else if( max_num == ( continue_counter_ref ) )return false;
      else{
       //std::cout<< " the_dose_index.x : " << the_dose_index.x <<std::endl;
       //std::cout<< " the_dose_index.y : " << the_dose_index.y <<std::endl;
       //std::cout<< " the_dose_index.z : " << the_dose_index.z <<std::endl;
    ...
    void PolygonInfo::CheckInside(  SnippetIndex current_moving_index , bool& it_is_inside_ref , bool& just_switched_inside_ref ){

      EdgeStatus previous_edge_status = edge_status;

      // Hier den edge status setzen

      CharacterizeIntersection characterize = pol_inf_vec.at( current_moving_index.edge ).at( current_moving_index.snip ).characterize;

      SetEdgeStatus( characterize );

      if( previous_edge_status !=  edge_status ){
        if( it_is_inside_ref ) it_is_inside_ref = false;
        else{
          it_is_inside_ref = true;
          just_switched_inside_ref = true;
        } 
      }     
      else just_switched_inside_ref = false;
    ...
    */
    ...
    bool PolygonInfo::SetCurrentStaticIndexKnownInsideOrOutside( SnippetIndex& current_static_index_ref , bool& nothing_to_do_ref ){
     ...
     /* std::cout<<  "  It is here !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  "  <<std::endl;
      ...
      if( got_it ) return inside;
      else assert( 0 ); // this must not happen
    ...
    bool PolygonInfo::CheckCornersOutside( SnippetIndex&  current_static_index_ref ){
      ...
      if( MaskFieldReceived->GetData( x, y, z ) == brightness_outside ) assert( 0 ); // this should never happen since we are dealing with a voxel that is touched by the structure. 
    ...
    bool PolygonInfo::CheckThereIsSuchNeighbour( Uint16 x , Uint16 y , Uint16 z , int shift_x , int shift_y  , int shift_z, field_content_type brightness ){

      Uint16 x_intern = x + shift_x;
      Uint16 y_intern = y;

      if( ( x_intern < MaskFieldReceived->GetDimX() ) && ( x_intern >= 0 ) ){ 
        if( MaskFieldReceived->GetData( x_intern, y_intern, z ) == brightness ) return true;
      }

      x_intern = x + shift_x;
      y_intern = y + shift_y;

      if( ( x_intern < MaskFieldReceived->GetDimX() ) && ( x_intern >= 0 ) && ( y_intern < MaskFieldReceived->GetDimY() ) && ( y_intern >= 0 ) ){
        if( MaskFieldReceived->GetData( x_intern, y_intern, z ) == brightness ) return true;
      }
    ...
    bool PolygonInfo::CheckCenterSurroundingStructure( bool& inside ){
      ...
      for( a_snippet_index.edge = 0 ; a_snippet_index.edge < 4 ; a_snippet_index.edge++ ){
        for( a_snippet_index.snip = 0 ; a_snippet_index.snip < ( pol_inf_vec.at( a_snippet_index.edge ).size() ) ; a_snippet_index.snip++ ){

          a_snippet = pol_inf_vec.at( a_snippet_index.edge ).at( a_snippet_index.snip );

          if(  !(  ( a_snippet.characterize == outside_outside_touches ) || ( a_snippet.characterize == edge_to_edge ) || ( a_snippet.characterize == corner ) ||  (a_snippet.characterize == outside_to_edge )  || ( a_snippet.characterize == edge_to_outside )   )   ){
    ...
    LegacyWorldCoordinate3D PolygonInfo::GetWorldCoordinate( LegacyWorldCoordinate3D voxel_coordinate ){
      ...
      world_coordinate.x = (orientation(0,0)*spacing.x*(voxel_coordinate.x-0.5)+orientation(0,1)*spacing.y*(voxel_coordinate.y-0.5)+posPatient.x);
      world_coordinate.y = (orientation(1,0)*spacing.x*(voxel_coordinate.x-0.5)+orientation(1,1)*spacing.y*(voxel_coordinate.y-0.5)+posPatient.y);
    ...
    bool PolygonInfo::SnippetIndexIsIdentical( SnippetIndex first_index , SnippetIndex second_index ){
      if( ( first_index.edge == second_index.edge ) && ( first_index.snip == second_index.snip ) ) return true;
      else return false;
    } 
    ...
    bool PolygonInfo::CheckPointOnEdgeOutside( SnippetIndex& current_static_index_ref , bool& inside ){
      ...
      for( a_snippet_index.edge = 0 ; a_snippet_index.edge < 4 ; a_snippet_index.edge++ ){
        ...
        for( a_snippet_index.snip = 0 ; a_snippet_index.snip < ( pol_inf_vec.at( a_snippet_index.edge ).size() ) ; a_snippet_index.snip++ ){   
          ...
          if( ( a_snippet.characterize == real_intersection ) || ( a_snippet.characterize == edge_to_inside )  || ( a_snippet.characterize == edge_to_outside )  || ( a_snippet.characterize == inside_inside_touches ) ){
            ...
            while( ( got_it == false ) && ( at_begin < 2 ) ){
              ...
              if( ( static_cast<float>( a_snippet.why_here.x ) != static_cast<float>( next_snippet.why_here.x ) ) || ( static_cast<float>( a_snippet.why_here.y ) != static_cast<float>( next_snippet.why_here.y ) )  ){
    ...
    void PolygonInfo::SetIndexIdentical(  IterativePolygonInTermsOfIntersections::WhichIntersection& intersection_index_to_ref , IterativePolygonInTermsOfIntersections::WhichIntersection& intersection_index_from_ref  ){
