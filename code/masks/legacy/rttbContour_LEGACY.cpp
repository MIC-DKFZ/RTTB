...
namespace rttb
{
	namespace masks
	{
		namespace legacy
		{
			...
			bool Contour::intersect(const LegacyWorldCoordinate3D& p1, const LegacyWorldCoordinate3D& p2,
			...
			double Contour::multiply(const LegacyWorldCoordinate3D& p1, const LegacyWorldCoordinate3D& p2,
			...
			LegacyWorldCoordinate2D Contour::calcInterPoint2D(const LegacyWorldCoordinate2D& p1,
			        ...
			        const LegacyWorldCoordinate2D& q2)
			{
				...
				else
				{
					...
					p.x = (WorldCoordinate)((std::min(std::max(p1.x, p2.x), std::max(q1.x,
					                                  q2.x)) + std::max(std::min(p1.x, p2.x), std::min(q1.x, q2.x))) / 2);
					p.y = (WorldCoordinate)((std::min(std::max(p1.y, p2.y), std::max(q1.y,
					                                  q2.y)) + std::max(std::min(p1.y, p2.y), std::min(q1.y, q2.y))) / 2);
			...
			LegacyWorldCoordinate3D Contour::calcInterPoint(const LegacyWorldCoordinate3D& p1,
			...
			Contour* Contour::calcIntersection(Contour& contour2)
			{
				...
				std::vector<LegacyWorldCoordinate3D>
				points1InContour2;//store points of this contour inside contour2 and all intersect points
				...
				for (it = segments1InBox2.begin(); it != segments1InBox2.end(); it++)
				{
					...
					for (it2 = segments2InBox1.begin(); it2 != segments2InBox1.end(); it2++)
					{
						...
						{
							LegacyWorldCoordinate3D interP = Contour::calcInterPoint(segment.begin, segment.end, (*it2).begin,
							                                 (*it2).end);
				...
				std::vector<LegacyWorldCoordinate3D>
				points2InContour1;//store points of contour2 inside this contour and all intersect points
