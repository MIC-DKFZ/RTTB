...
namespace rttb{
  namespace masks{
      ...
      typedef std::vector< std::vector<  std::vector< IterativePolygonInTermsOfIntersections::WhichIntersection > > > edge_intersections_intersection_coord_type;
      typedef std::vector< std::vector< IterativePolygonInTermsOfIntersections::WhichIntersection > > intersection_coord_type;
      typedef std::vector< std::vector<  IterativePolygonInTermsOfIntersections::WhichIntersection > >::iterator intersection_coord_iterator_type;
    ...
    public:
      ...
      PolygonInfo(){};
	  PolygonInfo( const LegacyPolygonType& the_polygon_in, int polygon_number_in, field_content_type brightness_outside_in, field_content_type brightness_inside_in , FieldOfScalar<field_content_type>* MaskFieldReceived_in , const PolygonInTermsOfIntersections& Polygon_Intersections_In , const LegacyDoseVoxelIndex3D& aDoseIndex , core::GeometricInfo& GInfIn, const IntersectionsThisVoxel& intersections_this_voxel_in  ); 

	  ~PolygonInfo(){};
      LegacyPolygonSequenceType& GetResultingSequence() { return resulting_sequence; }; 
    ...
    // success of this function is checked here. Calculation of the intersection points is checked as well in this process.
      bool TestCreateUnifiedListOfRawAndColumnIntersections( PolygonInTermsOfIntersections Expected_Unified_Polygon_Intersections );
      bool CompareCalculatedAndExpectedResultingSequenceForTest( LegacyPolygonSequenceType& the_resulting_sequence_ref );
    ...
    // representing the voxel edges. 
	  bool CheckResultingPolygonInfoVectorOfEdgesForTestStructure( PolygonInfoVectorOfEdges pol_inf_vec_expected );
      void ReferenceIterativePolygonForTest( IterativePolygonInTermsOfIntersections& it_poly_ref ){ it_poly_ref = it_poly; }
    ...
    private:
      ...
      void DoCharacterize( WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , CharacterizeIntersection& charact_ref , std::vector<WhichSector> anti_outside_combination_vector , WhichSector& inside_ref , WhichSector& preceding_edge_ref , WhichSector& following_edge_ref );
      ...
      void DoAppendIntermediateEdgeUpToThisSnippet( PolygonSnippet& snipp_ref , LegacyWorldCoordinate2D why_here , int the_previous_edge , int the_current_edge );   
      int EdgeDist( int edge_old , int the_current_edge );
      void IncrementEdge( int& edge_increment );
      bool GetCornerFromVoxelIntersections( unsigned int the_index, SnippetIndex edge_index , IterativePolygonInTermsOfIntersections::WhichIntersection& intersect_index );
      int GetNumberOfSnppetsThisEdge( SnippetIndex edge_index );
      bool SetToNextIntersectonAlongThisEdgeAndReturnDistanceBasedOnVoxelIntersections( LegacyWorldCoordinate2D& closest_local_current_position_ref , int current_edge, VoxelIntersectionIndex next_voxel_intersection );
      ...
      void WorkWithForwardAndBackwardEdgeSnippet( int which_one, float& angle_charact_a , float& angle_charact_b , PolygonSnippet& forward_snippet_ref , PolygonSnippet& backward_snippet_ref , PolygonSnippet& edge_snippet_ref , std::vector<PolygonSnippet>& snip_vec_local , int& erased  );     
      void WorkWithForwardEdgeSnippet( LegacyWorldCoordinate2D edge_coord, LegacyWorldCoordinate3D contour_point_one, int which_one, float& angle_charact_a , float& angle_charact_b , PolygonSnippet& forward_snippet_ref , PolygonSnippet& backward_snippet_ref , PolygonSnippet& edge_snippet_ref , std::vector<PolygonSnippet>& snip_vec_local , int& erased  );    
      void WorkWithBackwardEdgeSnippet( LegacyWorldCoordinate2D edge_coord, LegacyWorldCoordinate3D  contour_point_two , int which_one, float& angle_charact_a , float& angle_charact_b , PolygonSnippet& forward_snippet_ref , PolygonSnippet& backward_snippet_ref , PolygonSnippet& edge_snippet_ref , std::vector<PolygonSnippet>& snip_vec_local , int& erased  );    
      


    // The following function is just for test and easy debugging. 
	  void GetSectorsAndSetCharacteristicsDoubleCheck( SnippetIndex edge_index, LegacyWorldCoordinate2D edge_coord , LegacyWorldCoordinate3D& contour_point_one , LegacyWorldCoordinate3D& contour_point_two );      
      void GetSectors( LegacyWorldCoordinate2D edge_coord , LegacyWorldCoordinate3D& contour_point_one , LegacyWorldCoordinate3D& contour_point_two, WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref );  
      void GetSectorFirstPoint( LegacyWorldCoordinate2D edge_coord , LegacyWorldCoordinate3D& contour_point_one , WhichSector& sector_first_point_ref );     
    ...
    // wobei GetSectorFirstPoint dann in GetSectorThisPoint umbenannt werden sollte.
	  void GetSectorSecondPoint( LegacyWorldCoordinate2D edge_coord , LegacyWorldCoordinate3D& contour_point_two, WhichSector& sector_second_point_ref );     
      CharacterizeIntersection GetCharacteristicFirstCorner(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref  );  
      void GetAntiOutsideCombinationFirstCorner( std::vector<WhichSector>& anti_outside_combination_vector_ref );  
      ...
      bool BothOnEdge(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& preceding_edge_ref , WhichSector& following_edge_ref );         
      bool FromEdgeIn(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& preceding_edge_ref , WhichSector& inside_ref );  
      bool FromEdgeOut(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& preceding_edge_ref ,  std::vector<WhichSector> second_sector_vector  );  
      bool OutToEdge(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& following_edge_ref ,  std::vector<WhichSector> second_sector_vector );  
      bool InToEdge(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& following_edge_ref , WhichSector& inside_ref );  
      bool BothIn(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& inside_ref );  
      bool BothOut(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , std::vector<WhichSector> sector_vector );  
      bool OneInOneOut(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref , WhichSector& inside_ref , std::vector<WhichSector> first_sector_vector );  
      bool FirstSectorIsOneOfTheseAndSecondSectorIsOneOfThose(  WhichSector& sector_first_point_ref , std::vector<WhichSector> first_sector_vector , WhichSector& sector_second_point_ref , std::vector<WhichSector> second_sector_vector );  
      bool FirstSectorIsOneOfTheseAndSecondSectorIsNoneOfThose(  WhichSector& sector_first_point_ref , std::vector<WhichSector> first_sector_vector , WhichSector& sector_second_point_ref , std::vector<WhichSector> second_sector_vector );  
      bool FirstSectorIsNoneOfTheseAndSecondSectorIsNoneOfThose(  WhichSector& sector_first_point_ref , std::vector<WhichSector> first_sector_vector , WhichSector& sector_second_point_ref , std::vector<WhichSector> second_sector_vector );  
      CharacterizeIntersection GetCharacteristicSecondCorner( WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref  );  
      void GetAntiOutsideCombinationSecondCorner( std::vector<WhichSector>& anti_outside_combination_vector_ref );  
      ...
      CharacterizeIntersection GetCharacteristicThirdCorner(  WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref  );              
      void GetAntiOutsideCombinationThirdCorner( std::vector<WhichSector>& anti_outside_combination_vector_ref );
      ...
      CharacterizeIntersection GetCharacteristicFourthCorner( WhichSector& sector_first_point_ref ,  WhichSector& sector_second_point_ref  );  
      void GetAntiOutsideCombinationFourthCorner( std::vector<WhichSector>& anti_outside_combination_vector_ref );  
      ...
      bool AppendNextIntersectionAndCorrespoindingPolygonSnippets( snippet_vector& snip_vec_ref_local , snippet_vector& snip_vec_ref , int current_edge, VoxelIntersectionIndex& next_voxel_intersection  );  
      float SetToNextIntersectionAlongThisEdgeAndReturnDistance( LegacyWorldCoordinate2D& closest_local_current_position_ref , float distance_current_position , int current_edge );  
      bool IsTheSameEdgeButNoCorner( LegacyWorldCoordinate2D local_current_position , LegacyWorldCoordinate2D current_position , int current_edge );  
      bool AddSnippetsThisIntersection( snippet_vector& snip_vec_ref , LegacyWorldCoordinate2D closest_local_current_position );  
      ...
      void GetAngle( LegacyWorldCoordinate2D the_next_point ,  LegacyWorldCoordinate2D the_first_point , float& angle_charact );  
      ...
      float GetDistanceAlongEdge( LegacyDoseVoxelIndex3D edge_position , LegacyWorldCoordinate2D local_position  );  
      ...
      void GoGetPoly( bool& do_continue_ref , bool& next_is_edge_ref, int& continue_counter_ref , int& max_num );
      ...
      void GoAlongSnippetToNextIntersectionAndAppendToResultingSequence( PolygonSnippet& snipp_ref ,  int& continue_counter_ref );  
      ...
      void FindCorrespondingOneAndSetMovingIndexAndSetContinueCounter(  PolygonSnippet& snipp_ref , int& continue_counter_ref );  
      ...
      void CheckInside(  SnippetIndex current_moving_index , bool& it_is_inside_ref , bool& inside_switch_ref );  
      void SetEdgeStatus( CharacterizeIntersection characterize );  
      bool SetCurrentStaticIndexKnownInsideOrOutside( SnippetIndex& current_static_index_ref , bool& nothing_to_do_ref );  
      ...
      bool CheckThereIsSuchNeighbour( Uint16 x , Uint16 y , Uint16 z , int shift_x , int shift_y  , int shift_z , field_content_type brightness );  
      ...
      void SetIndexIdentical( IterativePolygonInTermsOfIntersections::WhichIntersection& intersection_index_to_ref , IterativePolygonInTermsOfIntersections::WhichIntersection& intersection_index_from_ref  );    
      ...
      void IncrementVoxelIntersectionIndex( int current_edge, VoxelIntersectionIndex& vox_inters_index , bool& changed_location );
