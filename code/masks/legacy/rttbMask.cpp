		...
		namespace legacy
		{
			...
			Mask::Mask(DoseIteratorInterface* aDoseIter  , StructureLegacy* aStructure)
			{
				...
				try
				{
					...
					{
						throw rttb::masks::SelfIntersectingStructureException(" This contour is not okay. It interscts itself. That doesn't make sense in radiotherapy.  ");
					}
			...
			*/
			...
			{
				if (FieldOfIntersections != NULL)
				{
					FieldOfIntersections->~FieldOfPointer();
				}

				for (int i = 0 ; i < MaskField.size() ; i++)
				{
					if (MaskField.at(i) != NULL)
					{
						MaskField.at(i)->~FieldOfScalar();
					}
				}
			...
			*/
			...
			void Mask::calcDoseData()
			{
				...
				while (_doseIter->hasNextVoxel() && it != _doseVoxelInStructure.end())
				{
					...
					if (count == doseVoxel.z * GInf.at(0).getNumColumns()*GInf.at(0).getNumRows() + doseVoxel.y *
					    GInf.at(0).getNumColumns() + doseVoxel.x)
					{
				...
				for (int resolution_level = (GInf.size() - 1) ; resolution_level >= 0  ; resolution_level--)
				{
					...
					for (int index_z = 0 ; index_z < GInf.at(resolution_level).getNumSlices() ; index_z++)
					{
						...
						for (int resolution_level = (GInf.size() - 1) ; resolution_level >= 0  ; resolution_level--)
						{
							...
							if ((MarkInterestingArea.at(resolution_level).at(index_z).index_end.x != 0)
							    && (MarkInterestingArea.at(resolution_level).at(index_z).index_end.y != 0))
							{

								for (index_internal.x = MarkInterestingArea.at(resolution_level).at(index_z).index_begin.x ;
								     index_internal.x <= MarkInterestingArea.at(resolution_level).at(index_z).index_end.x  ;
								     index_internal.x++)
								{
									for (index_internal.y = MarkInterestingArea.at(resolution_level).at(index_z).index_begin.y ;
									     index_internal.y <= MarkInterestingArea.at(resolution_level).at(index_z).index_end.y  ;
									     index_internal.y++)
									{


										if (MaskField.at(resolution_level)->GetData(index_internal.x, index_internal.y,
										        index_internal.z) == brightness_inside)
										{
										...
										if (MaskField.at(resolution_level)->GetData(index_internal.x, index_internal.y,
										        index_internal.z) == brightness_outside)
										{
										...
										if (MaskField.at(resolution_level)->GetData(index_internal.x, index_internal.y,
										        index_internal.z) == brightness_border)
										{

											CheckUppwardOneOutOfThisBlockHasSameIntensity(resolution_level , index_internal ,
											        brightness_border);

										}

										if (MaskField.at(resolution_level)->GetData(index_internal.x, index_internal.y,
										        index_internal.z) == brightness_unknown)
										{
											assert(0);    // This should not happen since each voxel must be regarded.
										}
			...
			void Mask::CheckUppwardBlockSameIntensity(int resolution_level_in , LegacyUnsignedIndex3D index_in ,
			        int brightness)
			{
				...
				if (resolution_level_in > 0)
				{
					...
					for (int resolution_level = (resolution_level_in - 1) ; resolution_level >= 0  ; resolution_level--)
					{
						...
						if ((index_internal.x >= MarkInterestingArea.at(resolution_level).at(
						         index_internal.z).index_begin.x)
						    && (index_internal.x <= (MarkInterestingArea.at(resolution_level).at(index_internal.z).index_end.x -
						                             block_length))
						    && (index_internal.y >= MarkInterestingArea.at(resolution_level).at(index_internal.z).index_begin.y)
						    && (index_internal.y <= (MarkInterestingArea.at(resolution_level).at(index_internal.z).index_end.y -
						                             block_length)))
						{
							if ((index_in.x >= MarkInterestingArea.at(resolution_level_in).at(index_in.z).index_begin.x)
							    && (index_in.x <= (MarkInterestingArea.at(resolution_level_in).at(index_in.z).index_end.x -
							                       block_length))
							    && (index_in.y >= MarkInterestingArea.at(resolution_level_in).at(index_in.z).index_begin.y)
							    && (index_in.y <= (MarkInterestingArea.at(resolution_level_in).at(index_in.z).index_end.y -
							                       block_length)))
							{

								bool its_fine = MaskField.at(resolution_level)->CheckBlockSameIntensity(index_internal.x ,
								                index_internal.y, index_internal.z , block_length , brightness);
			...
			void Mask::SetUppwardBlockThisIntensity(int resolution_level_in , LegacyUnsignedIndex3D index_in ,
			                                        int brightness)
			{
					...
					for (int resolution_level = (resolution_level_in - 1) ; resolution_level >= 0  ; resolution_level--)
					{
						                             ...
						                             block_length)))
						{
							                       ...
							                       block_length)))
							{
								...
								MaskField.at(resolution_level)->SetBlockThisIntensity(index_internal.x , index_internal.y,
								        index_internal.z , block_length , brightness);
			...
			void Mask::CheckUppwardOneOutOfThisBlockHasSameIntensity(int resolution_level_in ,
			        LegacyUnsignedIndex3D index_in , int brightness)
			{
					...
					for (int resolution_level = (resolution_level_in - 1) ; resolution_level >= 0  ; resolution_level--)
					{
						                             ...
						                             block_length)))
						{
							                       ...
							                       block_length)))
							{
								...
								bool its_fine = MaskField.at(resolution_level)->CheckOneOutOfThisBlockHasSameIntensity(
								                    index_internal.x , index_internal.y, index_internal.z , block_length , brightness);
			...
			bool Mask::DoesIntersect(int struct_index_a , int struct_index_b) const
			{
				...
				for (; itA != pitoiA.end() ; itA++)
				{
					...
					{
						secondPoint = pitoiA.at(0).contour_point_voxel_coord;
					}
					...
					for (; itB != pitoiB.end() ; itB++)
					{
						...
						if (i != j)
						{
							...
							if (HasIntersectionsThisPair(firstPoint , secondPoint, thirdPoint , fourthPoint))
							{
								return true;
							}
			...
			bool Mask::HasIntersectionsThisPair(const LegacyWorldCoordinate3D& firstPoint ,
			                                    ...
			                                    const LegacyWorldCoordinate3D& fourthPoint) const
			{
				...
				if (((secondPoint.y - firstPoint.y) == 0) || ((secondPoint.x - firstPoint.x) == 0))
				{
					assert(0);
				}


				if ((((fourthPoint.x - thirdPoint.x) / (secondPoint.x - firstPoint.x))  - ((
				            fourthPoint.y - thirdPoint.y) / (secondPoint.y - firstPoint.y)))  == 0)
				{
				...
				float  beta      = (((thirdPoint.y - firstPoint.y) / (secondPoint.y - firstPoint.y))   - ((
				                        thirdPoint.x - firstPoint.x) / (secondPoint.x - firstPoint.x)))           / (((
				                                    fourthPoint.x - thirdPoint.x) / (secondPoint.x - firstPoint.x))  - ((fourthPoint.y -
				                                            thirdPoint.y) / (secondPoint.y - firstPoint.y)))      ;
				float  alpha     = ((fourthPoint.x - thirdPoint.x) * beta +  thirdPoint.x - firstPoint.x) /
				                   (secondPoint.x - firstPoint.x);

				if (((beta > 0) && (beta < 1))  && ((alpha > 0)
				                                    && (alpha < 1)))          // falls diese Bedingung nicht erfuellt ist, gibt es zwar einen Schnittpunkt der Geraden, aber nicht im relevanten Bereich
				{
			...
			void Mask::FloodFill4(UnsignedIndexList indexList, int oldcolor, int newcolor, int resolution_level)
			{
				...
				while (!indexList.empty())
				{
					...
					{
						//insert index
						if (frontIndex.y + 1 <= MarkInterestingArea.at(resolution_level).at(frontIndex.z).index_end.y
						    && MaskField.at(resolution_level)->GetData(frontIndex.x, frontIndex.y + 1,
						            frontIndex.z) == oldcolor)
						{
						...
						if (frontIndex.y - 1 >= MarkInterestingArea.at(resolution_level).at(frontIndex.z).index_begin.y
						    && MaskField.at(resolution_level)->GetData(frontIndex.x, frontIndex.y - 1,
						            frontIndex.z) == oldcolor)
						{
							index.x = frontIndex.x;
						...
						if (frontIndex.x - 1 >= MarkInterestingArea.at(resolution_level).at(frontIndex.z).index_begin.x
						    && MaskField.at(resolution_level)->GetData(frontIndex.x - 1, frontIndex.y,
						            frontIndex.z) == oldcolor)
						{
						...
						if (frontIndex.x + 1 <= MarkInterestingArea.at(resolution_level).at(frontIndex.z).index_end.x
						    && MaskField.at(resolution_level)->GetData(frontIndex.x + 1, frontIndex.y,
						            frontIndex.z) == oldcolor)
						{
							index.x = frontIndex.x + 1;
			...
			void Mask::setInsideVoxel(int resolution_level , std::vector< UnsignedIndexList > indexListInside,
			                          std::vector< UnsignedIndexList > indexListOutside)
			{
				...
				for (index.z = 0 ; index.z < GInf.at(resolution_level).getNumSlices() ; index.z++)
				{
					...
					if (indexListOutside.at(index.z).size() == 0)
					{
						...
						for (index2.x = MarkInterestingArea.at(resolution_level).at(index.z).index_begin.x ;
						     index2.x <= MarkInterestingArea.at(resolution_level).at(index.z).index_end.x
						     && !firstVoxelOutStr  ; index2.x++)
						{
							for (index2.y = MarkInterestingArea.at(resolution_level).at(index.z).index_begin.y ;
							     index2.y <= MarkInterestingArea.at(resolution_level).at(index.z).index_end.y
							     && !firstVoxelOutStr  ; index2.y++)
							{
					...
					if (indexListOutside.at(index.z).size() > 0)
					{
						this->FloodFill4(indexListOutside.at(index.z), brightness_unknown, brightness_outside,
						                 resolution_level);
					}
					...
					if (indexListInside.at(index.z).size() == 0)
					{
						...
						for (int i = 0; i < sliceV.size(); i++)
						{
							...
							for (index2.x = MarkInterestingArea.at(resolution_level).at(index.z).index_begin.x ;
							     index2.x <= MarkInterestingArea.at(resolution_level).at(index.z).index_end.x
							     && !firstVoxelInStr  ; index2.x++)
							{

								for (index2.y = MarkInterestingArea.at(resolution_level).at(index.z).index_begin.y ;
								     index2.y <= MarkInterestingArea.at(resolution_level).at(index.z).index_end.y
								     && !firstVoxelInStr  ; index2.y++)
								{
									if (MaskField.at(resolution_level)->GetData(index2.x, index2.y, index2.z) == brightness_unknown)
									{
					...
					if (indexListInside.at(index.z).size() > 0)
					{
						this->FloodFill4(indexListInside.at(index.z), brightness_unknown, brightness_inside,
						                 resolution_level);
					}
					...
					for (index2.x = MarkInterestingArea.at(resolution_level).at(index.z).index_begin.x ;
					     index2.x <= MarkInterestingArea.at(resolution_level).at(index.z).index_end.x   ; index2.x++)
					{
						for (index2.y = MarkInterestingArea.at(resolution_level).at(index.z).index_begin.y ;
						     index2.y <= MarkInterestingArea.at(resolution_level).at(index.z).index_end.y   ; index2.y++)
						{
							if (MaskField.at(resolution_level)->GetData(index2.x, index2.y, index2.z) == brightness_unknown)
							{
								//std::cout << index2.toString()<<"---";
				...
				for (int x = 0 ; x < MaskField.at(resolution_level)->GetDimX() ; x++)
				{
					for (int y = 0 ; y < MaskField.at(resolution_level)->GetDimY() ; y++)
					{
						for (int z = 0 ; z < MaskField.at(resolution_level)->GetDimZ() ; z++)
						{
							...
							if (MaskField.at(resolution_level)->GetData(x, y, z) == brightness_inside)
							{
								counter_inside++;
							}

							if (MaskField.at(resolution_level)->GetData(x, y, z) == brightness_outside)
							{
								counter_outside++;
							}
							...
							if (MaskField.at(resolution_level)->GetData(x, y, z) == brightness_unknown)
							{
								counter_unknown++;
							}
			...
			void Mask::setInsideVoxelMultiRes(int resolution_level)
			{
				...
				if (resolution_level < (MaskField.size() - 1))
				{
					MaskField.at(resolution_level)->GetBorderRegion(MarkInterestingArea.at(resolution_level),
					        indexListInside , indexListOutside, brightness_unknown, brightness_inside, brightness_outside);
				}
							     ...
							     index_internal.x++)
							{
								     ...
								     index_internal.y++)
								{
									...
									if (MaskField.at(resolution_level)->GetData(index_internal.x, index_internal.y,
									        index_internal.z) != brightness_border)
									{
										MaskField.at(resolution_level)->PutData(index_internal.x, index_internal.y, index_internal.z,
										                                        brightness_unknown);
										//MaskField.at( resolution_level )->PutData( index_internal.x, index_internal.y, index_internal.z, brightness_inside );
			...
			void Mask::SeperateRegionsQuick()
			{
				...
				for (index.z = 0 ; index.z < GInf.at(0).getNumSlices() ; index.z++)
				{
					...
					if ((MarkInterestingArea.at(0).at(index.z).index_end.x != 0)
					    && (MarkInterestingArea.at(0).at(index.z).index_end.y != 0))
					{

						for (index_internal.x = MarkInterestingArea.at(0).at(index.z).index_begin.x ;
						     index_internal.x <= MarkInterestingArea.at(0).at(index.z).index_end.x  ; index_internal.x++)
						{
							for (index_internal.y = MarkInterestingArea.at(0).at(index.z).index_begin.y ;
							     index_internal.y <= MarkInterestingArea.at(0).at(index.z).index_end.y  ; index_internal.y++)
							{

								if (MaskField.at(0)->GetData(index_internal.x, index_internal.y,
								                             index_internal.z) != brightness_border)
								{
					...
					for (index.x = MarkInterestingArea.at(0).at(index.z).index_begin.x ;
					     index.x <= MarkInterestingArea.at(0).at(index.z).index_end.x ; index.x++)
					{
						...
						if (MaskField.at(0)->GetData(index.x, index.y, index.z) == brightness_inside)
						{
							...
							ConnectArea(brightness_inside , brightness_outside , index ,
							            MarkInterestingArea.at(0).at(index.z).index_begin ,
							            MarkInterestingArea.at(0).at(index.z).index_end);
						}
					...
					for (index.y = MarkInterestingArea.at(0).at(index.z).index_begin.y ;
					     index.y <= MarkInterestingArea.at(0).at(index.z).index_end.y  ; index.y++)
					{
			...
			void Mask::ConnectArea(field_content_type brightness_from , field_content_type brightness_to ,
			                       LegacyUnsignedIndex3D start_index, LegacyDoseVoxelIndex2D index_begin,
			                       LegacyDoseVoxelIndex2D index_end)
			{

				if ((start_index.x >= index_begin.x) && (start_index.y >= index_begin.y)
				    && (start_index.x <= index_end.x) && (start_index.y <= index_end.y))
				{
			...
			void Mask::SetThisIntersectionToCornerOrEdge(unsigned int index_x , unsigned int index_y ,
			        ...
			        IntersectionsThisVoxel& voxel_intersections)
			{
				...
				if (edge_number == 0)
				{
					SetThisIntersection(index_x , (index_x + 1) , index_y ,  intersection , coord2D.x, coord2D.y ,
					                    voxel_intersections, edge_number);
				}
				else if (edge_number == 1)
				{
					SetThisIntersection(index_y , (index_y + 1) , (index_x + 1) ,  intersection , coord2D.y, coord2D.x ,
					                    voxel_intersections, edge_number);
					//SetThisIntersectionYIncreasing( index_x , index_y , intersection , coord2D , voxel_intersections );
				}
				else if (edge_number == 2)
				{
					SetThisIntersection((index_x + 1) , index_x , (index_y + 1) ,  intersection , coord2D.x, coord2D.y ,
					                    voxel_intersections, edge_number);
					//SetThisIntersectionXDecreasing( index_x , index_y , intersection , coord2D , voxel_intersections );
				}
				else if (edge_number == 3)
				{
					SetThisIntersection((index_y + 1) , index_y , index_x , intersection , coord2D.y, coord2D.x ,
					                    voxel_intersections, edge_number);
			...
			void Mask::SetThisIntersection(unsigned int corner_of_interest , unsigned int corner_next,
			                               ...
			                               unsigned int edge_number)
			{
				...
				if (coord_fixed != static_cast<double>(corner_fixed))
				{
					assert(0);    // this should never happen since coord_fixed is the coordinate which is the same all along the edge that starts with corner_fixed
				}


				if (coord_of_interest == static_cast<double>
				    (corner_of_interest))    // falls der Schnittpunkt direkt am Anfang der Kante liegt also der Anfangspunkt der Kante ist
				{

					voxel_intersections.corner_intersections_intersection_coord.at(edge_number).push_back(
					    intersection);  // dann haenge den Schnittpunkt an die Liste der Schnittpunkte mit eben diesem Anfangspunkt der Kante an
					return;
				...
				else
				{
					...
					if (voxel_intersections.edge_intersections_intersection_coord.at(edge_number).size() >
					    0)    // falls die Kante schon Schnittpunkte hat
					{
						double first_coord_double = voxel_intersections.edge_intersections_vox_coord.at(edge_number).at(
						                                0).at(0);
						...
						if (CompareIfBetweenInsertAtBeginning(coord_of_interest , static_cast<double>(corner_of_interest) ,
						                                      first_coord_double , edge_number, intersection , voxel_intersections))
						{
							return;
						}
					}
					...
					if (CheckIfIndenticalOrBetweenRespectiveVectorElementsAndDoInsert(coord_of_interest , edge_number,
					...
					if (voxel_intersections.edge_intersections_vox_coord.at(edge_number).size() > 0)
					{
						that_coord_double = voxel_intersections.edge_intersections_vox_coord.at(edge_number).at((
						                        voxel_intersections.edge_intersections_intersection_coord.at(edge_number).size() - 1)).at(0) ;
					}
					else
					{
						that_coord_double = static_cast<double>(corner_of_interest);
					}

					if (that_coord_double == coord_of_interest)
					{

						if (coord_of_interest  ==  static_cast<double>(corner_of_interest))
						{
							assert(0);    // Das kann nicht sein, denn falls der Schnittpunkt wirklich der erste Punkt der Kante waere, muesste das ganz zu Beginn dieser Funktion bereits erkannt worden sein und die Funktion haette dann bereits returned. Falls that_coord_double also coord_of_interest (eizusortierender Punk) ist, kann dieser nicht der Anfangspunkt der Kante sein.
						}
						...
						voxel_intersections.edge_intersections_vox_coord.at(edge_number).at((
						            voxel_intersections.edge_intersections_intersection_coord.at(edge_number).size() - 1)).push_back(
						                coord_of_interest);
						voxel_intersections.edge_intersections_intersection_coord.at(edge_number).at((
						            voxel_intersections.edge_intersections_intersection_coord.at(edge_number).size() - 1)).push_back(
						                intersection);
						return;
					...
					else
					{
						...
						if (CompareIfBetweenAppend(coord_of_interest , that_coord_double, static_cast<double>(corner_next),
						...
						if (coord_of_interest == static_cast<double>(corner_next))
						{
							assert(0);    // this case is not supposed to be considered here. Its the next edge and should have gone elsewhere. Das Ende der Kante ist der Anfang der naechsten Kante uns haette daher bei dieser naechsten Kante landen muessen.
						}
					}
			...
			void Mask::SetThisIntersectionXIncreasing(unsigned int index_x , unsigned int index_y ,
			        IterativePolygonInTermsOfIntersections::WhichIntersection intersection ,
			        LegacyWorldCoordinate2D coord2D , IntersectionsThisVoxel& voxel_intersections ,
			        unsigned int edge_number)
			{

				if (coord2D.y != static_cast<double>(index_y))
				{
				...
				else
				{
					...
					if (voxel_intersections.edge_intersections_intersection_coord.at(0).size() > 0)
					{
						...
						if (CompareIfBetweenInsertAtBeginning(coord2D.x , static_cast<double>(index_x) ,
						                                      first_coord_double , edge_number, intersection , voxel_intersections))
						{
							return;
						}
					...
					if (CheckIfIndenticalOrBetweenRespectiveVectorElementsAndDoInsert(coord2D.x , edge_number,
					        intersection , voxel_intersections))
					{
						return;
					}
					...
					if (voxel_intersections.edge_intersections_vox_coord.at(0).size() > 0)
					{
						that_coord_double = voxel_intersections.edge_intersections_vox_coord.at(0).at((
						                        voxel_intersections.edge_intersections_intersection_coord.at(0).size() - 1)).at(0) ;
					}
					else
					{
						that_coord_double = static_cast<double>(index_x);
					}
					...
					if (that_coord_double == coord2D.x)
					{
						...
						voxel_intersections.edge_intersections_vox_coord.at(0).at((
						            voxel_intersections.edge_intersections_intersection_coord.at(0).size() - 1)).push_back(coord2D.x);
						voxel_intersections.edge_intersections_intersection_coord.at(0).at((
						            voxel_intersections.edge_intersections_intersection_coord.at(0).size() - 1)).push_back(
						                intersection);
						return;
					...
					else
					{
						...
						if (CompareIfBetweenAppend(coord2D.x , that_coord_double, static_cast<double>(index_x + 1),
						                           edge_number, intersection , voxel_intersections))
						{
							return;
						}

						if (coord2D.x == static_cast<double>(index_x + 1))
						{
							assert(0);    // this case is not supposed to be treated here.
						}
			...
			bool Mask::CompareIfBetweenAppend(double value_to_compare , double small_value_to_compare_with ,
			                                  double big_value_to_compare_with, unsigned int edge_number,
			                                  IterativePolygonInTermsOfIntersections::WhichIntersection intersection ,
			                                  IntersectionsThisVoxel& voxel_intersections)
			{

				if (((value_to_compare < big_value_to_compare_with)
				     && (value_to_compare >  small_value_to_compare_with))
				    || ((value_to_compare > big_value_to_compare_with)
				        && (value_to_compare <  small_value_to_compare_with)))
				{
					...
					voxel_intersections.edge_intersections_intersection_coord.at(edge_number).push_back(
					    intersection_vec);
					return true;
			...
			bool Mask::CheckIfIndenticalOrBetweenRespectiveVectorElementsAndDoInsert(double value_to_compare ,
			        unsigned int edge_number, IterativePolygonInTermsOfIntersections::WhichIntersection intersection ,
			        IntersectionsThisVoxel& voxel_intersections)
			{

				for (int i = 0 ;
				     i < (static_cast<int>(voxel_intersections.edge_intersections_intersection_coord.at(
				                               edge_number).size()) - 1) ; i++)
				{

					double the_coord_double = voxel_intersections.edge_intersections_vox_coord.at(edge_number).at(i).at(
					                              0) ;
					...
					if (value_to_compare == the_coord_double)
					{
						...
						voxel_intersections.edge_intersections_intersection_coord.at(edge_number).at(i).push_back(
						    intersection);
						return true;
					...
					else
					{
						...
						double another_double = voxel_intersections.edge_intersections_vox_coord.at(edge_number).at(
						                            i + 1).at(0) ;

						if (((another_double > value_to_compare) && (the_coord_double < value_to_compare))
						    || ((another_double < value_to_compare) && (the_coord_double > value_to_compare)))
						{
							...
							std::vector< IterativePolygonInTermsOfIntersections::WhichIntersection > intersection_vec;
							intersection_vec.push_back(intersection);
							intersection_coord_iterator_type it = voxel_intersections.edge_intersections_intersection_coord.at(
							        edge_number).begin() + (i + 1);

							voxel_intersections.edge_intersections_vox_coord.at(edge_number).insert((
							            voxel_intersections.edge_intersections_vox_coord.at(edge_number).begin() + (i + 1)) , double_vec);
							voxel_intersections.edge_intersections_intersection_coord.at(edge_number).insert(it ,
							        intersection_vec);
			...
			bool Mask::CompareIfBetweenInsertAtBeginning(double value_to_compare ,
			        double small_value_to_compare_with , double big_value_to_compare_with, unsigned int edge_number,
			        IterativePolygonInTermsOfIntersections::WhichIntersection intersection ,
			        IntersectionsThisVoxel& voxel_intersections)
			{

				if (((value_to_compare > small_value_to_compare_with)
				     && (value_to_compare < big_value_to_compare_with))
				    || ((value_to_compare < small_value_to_compare_with)
				        && (value_to_compare > big_value_to_compare_with)))
				{
					...
					voxel_intersections.edge_intersections_vox_coord.at(edge_number).insert(
					    voxel_intersections.edge_intersections_vox_coord.at(edge_number).begin(), double_vec);
					voxel_intersections.edge_intersections_intersection_coord.at(edge_number).insert(it ,
					        intersection_vec);
			...
			void Mask::SetThisIntersectionYIncreasing(unsigned int index_x , unsigned int index_y ,
			        IterativePolygonInTermsOfIntersections::WhichIntersection intersection ,
			        LegacyWorldCoordinate2D coord2D , IntersectionsThisVoxel& voxel_intersections)
			{
			...
			void Mask::SetThisIntersectionXDecreasing(unsigned int index_x , unsigned int index_y ,
			        IterativePolygonInTermsOfIntersections::WhichIntersection intersection ,
			        LegacyWorldCoordinate2D coord2D , IntersectionsThisVoxel& voxel_intersections)
			{

				if (coord2D.y != static_cast<double>(index_y + 1))
				{
					assert(0);    // this should never happen
				}
			...
			void Mask::SetThisIntersectionYDecreasing(unsigned int index_x , unsigned int index_y ,
			        IterativePolygonInTermsOfIntersections::WhichIntersection intersection ,
			        LegacyWorldCoordinate2D coord2D , IntersectionsThisVoxel& voxel_intersections)
			{
			...
			void Mask::Init()
			{
				...
				{
					FieldOfIntersections = new rttb::masks::legacy::FieldOfPointer<VoxelIntersectionsVectorPointer>
					(GInf.at(0).getNumColumns() , GInf.at(0).getNumRows() , 1);
				}
				...
				MaskField.resize(0);
				rttb::masks::legacy::FieldOfScalar<field_content_type>* a_field_of_scalar = new
				rttb::masks::legacy::FieldOfScalar<field_content_type>(GInf.at(0).getNumColumns() ,
				        GInf.at(0).getNumRows() , GInf.at(0).getNumSlices());
			...
			void Mask::InitInterestingArea()
			{
				...
				for (doseVoxelIndex.z = 0 ; doseVoxelIndex.z < GInf.at(0).getNumSlices() ; doseVoxelIndex.z++)
				{
					...
					if (sliceV.size() > 0)
					{
						...
						if (large_enough_x >= GInf.at(0).getNumColumns())
						{
							large_enough_x = (GInf.at(0).getNumColumns() - 1);
						}
						...
						if (large_enough_y >= GInf.at(0).getNumRows())
						{
							large_enough_y = (GInf.at(0).getNumRows() - 1);
						}
			...
			void Mask::InitMultiResolution()
			{
				...
				while (done == false)
				{
					...
					int new_dim_x = MaskField.at(0)->GetDimX() / static_cast<int>(GetBlockLengthThisResolutionLevel(
					                    MaskField.size())) + 1;
					int new_dim_y = MaskField.at(0)->GetDimY() / static_cast<int>(GetBlockLengthThisResolutionLevel(
					                    MaskField.size())) + 1;
					...
					if (new_dim_x > 20 && new_dim_y > 20)
					{
						...
						rttb::masks::legacy::FieldOfScalar<field_content_type>* a_field_of_scalar = new
						rttb::masks::legacy::FieldOfScalar<field_content_type>(GInf.at(GInf.size() - 1).getNumColumns() ,
						        GInf.at(GInf.size() - 1).getNumRows() , GInf.at(GInf.size() - 1).getNumSlices());
						MaskField.push_back(a_field_of_scalar);
						...
						for (int z = 0 ; z < MarkInterestingArea.at(0).size() ; z++)
						{
							...
							if (NewArea.index_begin.x > 0)
							{
								NewArea.index_begin.x -= 1;
							}

							NewArea.index_begin.y = (MarkInterestingArea.at(0).at(z).index_begin.y / power);

							if (NewArea.index_begin.y > 0)
							{
								NewArea.index_begin.y -= 1;
							}

							NewArea.index_end.x = (MarkInterestingArea.at(0).at(z).index_end.x / power);
							NewArea.index_end.x += 1;

							if (NewArea.index_end.x >= MaskField.at(MaskField.size() - 1)->GetDimX())
							{
								NewArea.index_end.x = (MaskField.at(MaskField.size() - 1)->GetDimX() - 1);
							}

							NewArea.index_end.y = (MarkInterestingArea.at(0).at(z).index_end.y / power);
							NewArea.index_end.y += 1;

							if (NewArea.index_end.y >= MaskField.at(MaskField.size() - 1)->GetDimY())
							{
								NewArea.index_end.y = (MaskField.at(MaskField.size() - 1)->GetDimY() - 1);
							}
			...
			void Mask::InitIntersections()
			{
				...
				for (int counter = 1 ; counter < GInf.size() ; counter++)
				{
					...
					for (unsigned int i = 0 ; i < strVector.size() ; i++)
					{
						...
						for (unsigned int j = 0 ; j < strVector.at(i).size() ; j++)
						{
							...
							Intersections.at(counter).at(i).at(j).contour_point_voxel_coord.z = GetZIndex(
							            contour_point_world.z);
				...
				if (Intersections.size() != GInf.size())
				{
					assert(0);
				}
			...
			void Mask::GetIntersections(int resolution_level)
			{
				...
				for (unsigned int struct_index = 0 ; struct_index < strVector.size() ; struct_index++)
				{
					...
					for (int point_index = 0 ; point_index < (strVector.at(struct_index).size() - 1); point_index++)
					{
						...
						std::vector<LegacyWorldCoordinate2D>& raw_intersections_ref = Intersections.at(resolution_level).at(
						            struct_index).at(point_index).raw_intersections;
						std::vector<LegacyWorldCoordinate2D>& column_intersections_ref = Intersections.at(
						            resolution_level).at(struct_index).at(point_index).column_intersections;

						std::vector<LegacyWorldCoordinate2D>& intersections_raw_and_column_ref = Intersections.at(
						            resolution_level).at(struct_index).at(point_index).intersections_raw_and_column;
						...
						UnifyRawAndColumn(firstPoint , raw_intersections_ref , column_intersections_ref ,
						                  intersections_raw_and_column_ref, resolution_level);
					...
					if (strVector.at(struct_index).size() > 0)
					{
						...
						std::vector<LegacyWorldCoordinate2D>& raw_intersections_ref = Intersections.at(resolution_level).at(
						            struct_index).at((strVector.at(struct_index).size() - 1)).raw_intersections;
						std::vector<LegacyWorldCoordinate2D>& column_intersections_ref = Intersections.at(
						            resolution_level).at(struct_index).at((strVector.at(struct_index).size() - 1)).column_intersections;

						std::vector<LegacyWorldCoordinate2D>& intersections_raw_and_column_ref = Intersections.at(
						            resolution_level).at(struct_index).at((strVector.at(struct_index).size() -
						                    1)).intersections_raw_and_column;
			...
			void Mask::UnifyRawAndColumn(LegacyWorldCoordinate3D the_firstPoint,
			                             ...
			                             std::vector<LegacyWorldCoordinate2D>& intersections_raw_and_column_ref, int resolution_level)
			{
				...
				if ((intersections_raw_and_column_ref.size() ==
				     0))     // falls das nicht der Fall ist ist der Verlauf der Konturpunkte horizontal oder vertikal. Dann gibt es sowiso nur horizontale oder vertikale Schnittpunkte und die Vereinigung ist in GoIntersect... bereits geschehen.
				{
					...
					LegacyWorldCoordinate2D firstPoint = GetDoubleIndex2D(the_firstPoint.x , the_firstPoint.y,
					                                     resolution_level);
					...
					while (! done)
					{
						...
						{
							rawPoint = raw_intersections_ref.at(counter);
						}
						else if (raw_intersections_ref.size() > 0)
						{
							rawPoint = raw_intersections_ref.at(raw_intersections_ref.size() - 1);
						}

						if (internal_counter < column_intersections_ref.size())
						{
							columnPoint = column_intersections_ref.at(internal_counter);
						}
						else if (column_intersections_ref.size() > 0)
						{
							columnPoint = column_intersections_ref.at(column_intersections_ref.size() - 1);
						}

						double distance_raw = (firstPoint.y - rawPoint.y) * (firstPoint.y - rawPoint.y) +
						                      (firstPoint.x - rawPoint.x) * (firstPoint.x - rawPoint.x);
						double distance_column = (firstPoint.x - columnPoint.x) * (firstPoint.x - columnPoint.x) +
						                         (firstPoint.y - columnPoint.y) * (firstPoint.y - columnPoint.y);
						...
						if (distance_raw == distance_column)
						{
							...
							else
							{
								...
								counter++;
						          ...
						          && (internal_counter < column_intersections_ref.size()))
						         || (counter >= (raw_intersections_ref.size())))
						{
							intersections_raw_and_column_ref.push_back(columnPoint);
							internal_counter++;
						}
			...
			void Mask::GoIntersectRaw(LegacyWorldCoordinate3D firstPoint , LegacyWorldCoordinate3D secondPoint ,
				...
				if ((secondPoint.y - firstPoint.y) != 0)
				{
					if ((secondPoint.x - firstPoint.x) != 0)
					{
						...
						*/
						...
						double x_offset_raw = GInf.at(resolution_level).getImageOrientationColumn()(0) * GInf.at(
						                          resolution_level).getPixelSpacingColumn();
						double y_offset_raw = GInf.at(resolution_level).getImageOrientationColumn()(1) * GInf.at(
						                          resolution_level).getPixelSpacingColumn();
						...
						if (end_ind > (GInf.at(resolution_level).getNumRows() - 1))
						{
							end_ind = GInf.at(resolution_level).getNumRows() - 1;
						}
						...
						for (int raw_index = begin_ind ; raw_index <= end_ind ; raw_index++)
						{
							...
							the_index.x = multi_res_offset + (GInf.at(resolution_level).getImagePositionPatient()(
							                                      1) + y_offset_raw * (raw_index - multi_res_offset) - firstPoint.y  - (GInf.at(
							                                              resolution_level).getImagePositionPatient()(0) - firstPoint.x  + x_offset_raw *
							                                              (raw_index - multi_res_offset)) * ((secondPoint.y - firstPoint.y) / (secondPoint.x -
							                                                      firstPoint.x)))        / (((-1)  *  GInf.at(resolution_level).getImageOrientationRow()(1) * GInf.at(
							                                                              resolution_level).getPixelSpacingRow() + (secondPoint.y - firstPoint.y) /
							                                                              (secondPoint.x - firstPoint.x) * (GInf.at(resolution_level).getImageOrientationRow()(0) * GInf.at(
							                                                                      resolution_level).getPixelSpacingRow())));
							...
							double lambda = (GInf.at(resolution_level).getImagePositionPatient()(0) + x_offset_raw *
							                 (raw_index - multi_res_offset) +  GInf.at(resolution_level).getImageOrientationRow()(0) * GInf.at(
							                     resolution_level).getPixelSpacingRow() * (the_index.x - multi_res_offset) - firstPoint.x) /
							                (secondPoint.x - firstPoint.x) ;
					...
					{
						GoGetRawIntersectionsAlongThisColumn(firstPoint , secondPoint ,  raw_intersections_ref ,
						                                     resolution_level);
					}
			...
			void Mask::GoGetRawIntersectionsAlongThisColumn(LegacyWorldCoordinate3D firstPoint ,
			        ...
			        int resolution_level)
			{
				...
				if (index_diff > 0)
				{
					...
					if (floor(index_first_point.y) < index_first_point.y)
					{
						begin_ind += 1;
					}
				...
				else if (index_diff < 0)
				{
					...
					if (floor(index_second_point.y) < index_second_point.y)
					{
						begin_ind += 1;
					}
			...
			void Mask::GoIntersectColumn(LegacyWorldCoordinate3D firstPoint ,
			                             LegacyWorldCoordinate3D secondPoint ,
				...
				if ((secondPoint.x - firstPoint.x) != 0)
				{
					...
					if ((secondPoint.y - firstPoint.y) != 0)
					{
						...
						*/
						...
						double x_offset_column = GInf.at(resolution_level).getImageOrientationRow()(0) * GInf.at(
						                             resolution_level).getPixelSpacingRow();
						double y_offset_column = GInf.at(resolution_level).getImageOrientationRow()(1) * GInf.at(
						                             resolution_level).getPixelSpacingRow();
						...
						if (end_ind > (GInf.at(resolution_level).getNumColumns() - 1))
						{
							end_ind = GInf.at(resolution_level).getNumColumns() - 1;
						}
						...
						for (int column_index = begin_ind ; column_index <= end_ind ; column_index++)
						{
							...
							//neu multi res
							the_index.y = multi_res_offset + (firstPoint.y + (secondPoint.y - firstPoint.y) * (GInf.at(
							                                      resolution_level).getImagePositionPatient()(0) + x_offset_column * (column_index - multi_res_offset)
							                                  - firstPoint.x)  / (secondPoint.x - firstPoint.x) - GInf.at(
							                                      resolution_level).getImagePositionPatient()(1) + y_offset_column * (column_index -
							                                              multi_res_offset)) / (GInf.at(resolution_level).getImageOrientationColumn()(1) * GInf.at(
							                                                      resolution_level).getPixelSpacingColumn() - (GInf.at(resolution_level).getImageOrientationColumn()(
							                                                              0) * GInf.at(resolution_level).getPixelSpacingColumn()) * (secondPoint.y - firstPoint.y) /
							                                                      (secondPoint.x - firstPoint.x)) ;
							...
							double lambda = (GInf.at(resolution_level).getImagePositionPatient()(0) + x_offset_column *
							                 (column_index - multi_res_offset) + GInf.at(resolution_level).getImageOrientationColumn()(
							                     0) * GInf.at(resolution_level).getPixelSpacingColumn() * (the_index.y - multi_res_offset) -
							                 firstPoint.x)  / (secondPoint.x - firstPoint.x);
					...
					{
						GoGetColumnIntersectionsAlongThisRaw(firstPoint , secondPoint , column_intersections_ref,
						                                     resolution_level);
					}
			...
			bool Mask::TestGoIntersectColumnByComparisonWithExpectedResult(LegacyWorldCoordinate3D firstPoint ,
			        LegacyWorldCoordinate3D secondPoint ,
			        std::vector<LegacyWorldCoordinate2D>& column_intersections_ref,
			...
			bool Mask::TestGoIntersectRawByComparisonWithExpectedResult(LegacyWorldCoordinate3D firstPoint ,
			        LegacyWorldCoordinate3D secondPoint , std::vector<LegacyWorldCoordinate2D>& raw_intersections_ref,
			        std::vector<LegacyWorldCoordinate2D>& expected_intersections_ref)
			{
			...
			bool Mask::IdenticalIntersectionsForTest(std::vector<LegacyWorldCoordinate2D>&
			        calculated_intersections_ref , std::vector<LegacyWorldCoordinate2D>& expected_intersections_ref)
			{
				...
				if (calculated_intersections_ref.size() != expected_intersections_ref.size())
				{
					...
					std::cout << " calculated_intersections_ref.size()  :   "  << calculated_intersections_ref.size()
					          << std::endl;
					std::cout << " expected_intersections_ref.size()    :   "  << expected_intersections_ref.size()
					          << std::endl;
					...
					if (calculated_intersections_ref.size() < expected_intersections_ref.size())
					{
						...
						{
							std::cout << " expected_intersections_ref.at( counter ).x : " << expected_intersections_ref.at(
							              counter).x << std::endl;
							std::cout << " calculated_intersections_ref.at( counter ).x : " << calculated_intersections_ref.at(
							              counter).x << std::endl;
							std::cout << " expected_intersections_ref.at( counter ).y : " << expected_intersections_ref.at(
							              counter).y << std::endl;
							std::cout << "  calculated_intersections_ref.at( counter ).y : " <<
							          calculated_intersections_ref.at(counter).y << std::endl;
						}
					}
					else
					{
				...
				for (int counter = 0 ; counter < expected_intersections_ref.size() ; counter++)
				{
					...
					double diffa = ((expected_intersections_ref.at(counter).x - calculated_intersections_ref.at(
					                     counter).x) * (expected_intersections_ref.at(counter).x - calculated_intersections_ref.at(
					                                        counter).x));
					double diffb = ((expected_intersections_ref.at(counter).y - calculated_intersections_ref.at(
					                     counter).y) * (expected_intersections_ref.at(counter).y - calculated_intersections_ref.at(
					                                        counter).y));
			...
			void Mask::GoGetColumnIntersectionsAlongThisRaw(LegacyWorldCoordinate3D firstPoint ,
			        LegacyWorldCoordinate3D secondPoint ,
			        std::vector<LegacyWorldCoordinate2D>& column_intersections_ref, int resolution_level)
			{
				...
				if (index_diff > 0)
				{
					...
					if (floor(index_first_point.x) < index_first_point.x)
					{
						begin_ind += 1;
					}
				...
				else if (index_diff < 0)
				{
					...
					if (floor(index_second_point.x) < index_second_point.x)
					{
						begin_ind += 1;
					}
			...
			void Mask::ShowIntersections()
			{
				...
				for (unsigned int resolution_level = 0 ; resolution_level < Intersections.size() ;
				     resolution_level++)
				{

					std::cout << "  resolution_level : " <<  resolution_level << std::endl;

					for (unsigned int struct_index = 0 ; struct_index < Intersections.at(resolution_level).size() ;
					     struct_index++)
					{
						...
						for (unsigned int point_index = 0 ;
						     point_index < (Intersections.at(resolution_level).at(struct_index).size()); point_index++)
						{

							std::cout << " point_index : " << point_index << std::endl;

							for (unsigned int intersect_index = 0 ;
							     intersect_index <  Intersections.at(resolution_level).at(struct_index).at(
							         point_index).intersections_raw_and_column.size() ; intersect_index++)
							{

								std::cout << "  intersect_index : " << intersect_index << std::endl;

								coord2D = Intersections.at(resolution_level).at(struct_index).at(
								              point_index).intersections_raw_and_column.at(intersect_index);
			...
			void Mask::MarkVoxelsTouchedByStructure(int resolution_level)
			{
				     ...
				     struct_index++)
				{
					...
					for (unsigned int index_z = 0 ; index_z < GInf.at(resolution_level).getNumSlices() ; index_z++)
					{
						...
						{
							if (str_v.at(count) == struct_index)
							{
								wich_slice.push_back(index_z);
							}
					     ...
					     point_index < (Intersections.at(resolution_level).at(struct_index).size()); point_index++)
					{
						         ...
						         point_index).intersections_raw_and_column.size() ; intersect_index++)
						{
							...
							if ((coord2D.x >= 0) && (coord2D.y >= 0) && (coord2D.x < MaskField.at(resolution_level)->GetDimX())
							    && (coord2D.y < MaskField.at(resolution_level)->GetDimY()))
							{
								...
								for (unsigned int this_counter = 0 ; this_counter < wich_slice.size() ; this_counter++)
								{
									...
									if ((floor(coord2D.x) == coord2D.x) && (floor(coord2D.y) == coord2D.y) && (index_y > 0)
									    && (index_x > 0))
									{

										MaskField.at(resolution_level)->PutData((index_x - 1)  , (index_y - 1) , index_z ,
										                                        brightness_border);
						...
						for (unsigned int this_counter = 0 ; this_counter < wich_slice.size() ; this_counter++)
						{
							...
							int intx = floor(Intersections.at(resolution_level).at(struct_index).at(
							                     point_index).contour_point_voxel_coord.x);
							int inty = floor(Intersections.at(resolution_level).at(struct_index).at(
							                     point_index).contour_point_voxel_coord.y);


							if ((intx >= 0) && (inty >= 0) && (intx < MaskField.at(resolution_level)->GetDimX())
							    && (inty < MaskField.at(resolution_level)->GetDimY()))
							{
							    ...
							    && (coord2D.y < MaskField.at(resolution_level)->GetDimY()))
							{
								...
								if (FieldOfIntersections)
								{
									...
									if (floor(coord2D.y) == coord2D.y)
									{
										...
										AddToFieldOfIntersections(index_x , index_y , 0 , struct_index , point_index , intersect_index ,
										                          coord2D, voxel_side);
									}
								...
								if ((floor(coord2D.x) == coord2D.x) && (index_x > 0))
								{
									...
									if (FieldOfIntersections)   // Ecke wird gegebenenfalls mit angehaengt
									{
										...
										AddToFieldOfIntersections((index_x - 1) , index_y , 0 , struct_index , point_index ,
										                          intersect_index , coord2D, voxel_side);
									}
								...
								if ((floor(coord2D.y) == coord2D.y) && (index_y > 0))
								{
									...
									if (FieldOfIntersections)
									{
										...
										if (floor(coord2D.x) ==
										    coord2D.x)     // Ecke wird gegebenenfalls nicht mit angehaengt, deshalb Fallunterscheidunn noetig, damit sie angehaengt wird ( Ecke wird nur an die Kante angehaengt an deren Anfang sie steht)
										{
											unsigned int voxel_side = 3;
											AddToFieldOfIntersections(index_x , (index_y - 1) , 0 , struct_index , point_index ,
											                          intersect_index , coord2D, voxel_side);
										}
										else
										{
								    ...
								    && (index_x > 0))
								{
									...
									if (FieldOfIntersections)
									{
										...
										AddToFieldOfIntersections((index_x - 1) , (index_y - 1) , 0 , struct_index , point_index ,
										                          intersect_index , coord2D, voxel_side);
									}
			...
			void Mask::AddToFieldOfIntersections(unsigned int index_x , unsigned int index_y ,
			                                     unsigned int index_z , unsigned int struct_index , unsigned int point_index ,
			                                     unsigned int intersect_index , LegacyWorldCoordinate2D coord2D, unsigned int voxel_side)
			{
				...
				if (go)
				{
					...
					if (which_one != (-1))
					{
						...
						SetThisIntersectionToCornerOrEdge(index_x , index_y , which_intersection , coord2D , voxel_side,
						                                  FieldOfIntersections->GetData(index_x , index_y, index_z)->at(which_one));
			...
			bool Mask::IsItToBeRegarded(unsigned int struct_index , unsigned int point_index ,
			                            unsigned int intersect_index , int index_x , int index_y)
			{
				...
				if ((Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord.y !=
				     Intersections.at(0).at(struct_index).at(point_index).intersections_raw_and_column.at(
				         intersect_index).y) ||
				    (Intersections.at(0).at(struct_index).at(point_index).intersections_raw_and_column.at(
				         intersect_index).x  != Intersections.at(0).at(struct_index).at(
				         point_index).contour_point_voxel_coord.x))
				{
					return true;
				}
				else
				{
					...
					if (GetFirstDifferent(struct_index_other , point_index_other ,
					                      Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord))
					{



						if ((floor(Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord.y / 1.0)
						     != Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord.y)
						    && (floor(Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord.x / 1.0) ==
						        Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord.x))
						{

							if (Intersections.at(0).at(struct_index_other).at(point_index_other).contour_point_voxel_coord.x  ==
							    Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord.x)
							{
								return true;
							}
						...
						else if ((floor(Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord.y /
						                1) ==  Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord.y)
						         && (floor(Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord.x / 1) !=
						             Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord.x))
						{

							if (Intersections.at(0).at(struct_index_other).at(point_index_other).contour_point_voxel_coord.y ==
							    Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord.y)
							{
								return true;
							}
			...
			bool Mask::GetFirstDifferent(unsigned int& struct_index , unsigned int& point_index ,
			                             LegacyWorldCoordinate3D compare_point)
			{

				if (Intersections.at(0).at(struct_index).size() <= 1)
				{
					std::cerr <<  "  Doesn't make sense to voxelize contour with less than two points !  "  <<
					          std::endl;
				...
				while (docontinue)
				{
					...
					if ((Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord.x !=
					     compare_point.x)
					    || (Intersections.at(0).at(struct_index).at(point_index).contour_point_voxel_coord.y !=
					        compare_point.y))
					{
					...
					{
						point_index = (Intersections.at(0).at(struct_index).size() - 1);
					}

					counter++;

					if (counter > (Intersections.at(0).at(struct_index).size() * 2))
					{
						break;
					}
			...
			void Mask::ExtendFieldOfIntersections(unsigned int index_x , unsigned int index_y ,
			                                      unsigned int index_z , unsigned int struct_index)
			{
				...
				PolygonInTermsOfIntersections& Polygon_Intersections_Input_Ref = Intersections.at(0).at(
				            struct_index);
			...
			int Mask::GetRespectiveIntersections(unsigned int index_x , unsigned int index_y ,
			                                     unsigned int index_z , unsigned int struct_index) const
			{
				...
				if (FieldOfIntersections->GetData(index_x , index_y, index_z) != NULL)
				{
					...
					{
						if (FieldOfIntersections->GetData(index_x , index_y,
						                                  index_z)->at(i).GetPolygonNumber() == struct_index)
						{
							return i;
						}
					}
			...
			LegacyWorldCoordinate2D Mask::GetDoubleIndex2D(double x , double y, int resolution_level)
			{
				...
				double a = y -  GInf.at(resolution_level).getImagePositionPatient()(1) - GInf.at(
				               resolution_level).getImageOrientationRow()(1) * GInf.at(resolution_level).getPixelSpacingRow() *
				           (x - GInf.at(resolution_level).getImagePositionPatient()(0)) / (GInf.at(
				                       resolution_level).getImageOrientationRow()(0) * GInf.at(resolution_level).getPixelSpacingRow()) ;
				double b = GInf.at(resolution_level).getImageOrientationColumn()(1) * GInf.at(
				               resolution_level).getPixelSpacingColumn()  - GInf.at(resolution_level).getImageOrientationRow()(
				               1) * GInf.at(resolution_level).getPixelSpacingRow() * (GInf.at(
				                           resolution_level).getImageOrientationColumn()(0) * GInf.at(
				                           resolution_level).getPixelSpacingColumn()) / (GInf.at(resolution_level).getImageOrientationRow()(
				                                       0) * GInf.at(resolution_level).getPixelSpacingRow()) ;
				double c = (x - GInf.at(resolution_level).getImagePositionPatient()(0))  ;
				double d = GInf.at(resolution_level).getImageOrientationColumn()(0) * GInf.at(
				               resolution_level).getPixelSpacingColumn();
				double e = (GInf.at(resolution_level).getImageOrientationRow()(0) * GInf.at(
				                resolution_level).getPixelSpacingRow());
				...
				result.x = ((c  - (d * (a / b))) / e) + GetWorldCoordinateOffsetThisResolutionLevel(
				               resolution_level);
			...
			LegacyWorldCoordinate1D Mask::GetZIndex(LegacyWorldCoordinate1D z)
			{
				...
				//neu
				LegacyWorldCoordinate1D result = ((z  - GInf.at(0).getImagePositionPatient()(2)) / GInf.at(
				                                      0).getSliceThickness()) + 0.5;
				return result;
			}
			...
			bool Mask::DoubleCheckIsOnBorderForTest(LegacyDoseVoxelIndex3D& aDoseIndex)
			{
				...
				if (MaskField.at(resolution_level)->GetData(aDoseIndex.x, aDoseIndex.y,
				        aDoseIndex.z) == brightness_border)
				{
					return true;
				}
				else
			...
			bool Mask::CheckFractionThisVoxelCorrectForTest(LegacyDoseVoxelIndex3D another_dose_index ,
			        float the_correct_result)
			{
				if (GetFractionInside(another_dose_index) == the_correct_result)
				{
					return true;
				}
				else
				...
				if (do_detailed_subvox)
				{
					...
					if (got_it)
					{
						...
						if (which_one != (-1))
						{
							...
							IntersectionsThisVoxel intersections_this_voxel =  FieldOfIntersections->GetData(aDoseIndex.x ,
							        aDoseIndex.y , 0)->at(which_one);
							polInfTest = new PolygonInfo(the_polygon_ref, struct_index , brightness_outside ,
							                             brightness_inside , MaskField.at(0) , Polygon_Intersections_Input_Ref ,  aDoseIndex , GInf.at(0) ,
							                             intersections_this_voxel);
			...
			bool Mask::GetIntersectionsOfSliceForTest(unsigned int position ,
			...
			bool Mask::ReturnFirstWithIntersectionsForTest(unsigned int& position)
			{
				...
				while (position < Intersections.at(0).size())
				{
					...
					if (Intersections.at(0).at(position).size() > 0)
					{
						return true;
					}
					else
			...
			float Mask::GetFractionInside(const LegacyDoseVoxelIndex3D& aDoseIndex)
			{
				...
				for (unsigned int count = 0 ; count < str_v.size() ; count++)
				{
					...
					if (which_one != (-1))
					{
						...
						const PolygonInTermsOfIntersections& Polygon_Intersections_Input_Ref = Intersections.at(0).at(
						            struct_index);
						...
						const IntersectionsThisVoxel& intersections_this_voxel = FieldOfIntersections->GetData(
						            aDoseIndex.x , aDoseIndex.y , 0)->at(which_one) ;


						PolygonInfo polInf = PolygonInfo(the_polygon_ref , struct_index , brightness_outside ,
						                                 brightness_inside , MaskField.at(0) , Polygon_Intersections_Input_Ref ,  aDoseIndex , GInf.at(0) ,
						                                 intersections_this_voxel);
