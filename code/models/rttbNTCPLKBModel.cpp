...
namespace rttb{
	...
	namespace models{
		...
		const BioModelParamType NTCPLKBModel::getA(){return _a;}

		void NTCPLKBModel::setM(const BioModelParamType aM){
			_m=aM;
		}

		const BioModelParamType NTCPLKBModel::getM(){return _m;}
		...
		const int NTCPLKBModel::getParameterID(const std::string aParamName) const{
			if(aParamName=="d50")
				...
				{
		rttbExceptionMacro(core::InvalidParameterException, << "Parameter name "<<aParamName<<" invalid: it should be d50 or m or a!");
