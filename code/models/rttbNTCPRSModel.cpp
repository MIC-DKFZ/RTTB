...
namespace rttb{
	...
	namespace models{
		...
		void NTCPRSModel::setGamma(const BioModelParamType aGamma){_gamma=aGamma;}

		const BioModelParamType NTCPRSModel::getGamma(){return _gamma;}

		void NTCPRSModel::setS(const BioModelParamType aS){
			_s=aS;
		}

		const BioModelParamType NTCPRSModel::getS(){return _s;}
		...
		const int NTCPRSModel::getParameterID(const std::string aParamName) const{
			if(aParamName=="d50")
				...
				{
				rttbExceptionMacro(core::InvalidParameterException, << "Parameter name "<<aParamName<<" invalid: it should be d50 or gamma or s!");
				}
			}
