...
namespace rttb{
	...
	namespace models{
		...
		TCPLQModel::TCPLQModel(DVHPointer aDVH, BioModelParamType aRho, int aNumberOfFractions, BioModelParamType aAlpha_Beta, 
			BioModelParamType aAlphaMean, BioModelParamType aAlphaVariance): TCPModel(aDVH,aNumberOfFractions),_alphaMean(aAlphaMean), 
		...
		void TCPLQModel::setRho(const BioModelParamType aRho){_rho=aRho;}

		const BioModelParamType TCPLQModel::getAlpahBeta(){return _alpha_beta;}

		const BioModelParamType TCPLQModel::getAlphaMean(){return _alphaMean;}

		const BioModelParamType TCPLQModel::getAlphaVariance(){return _alphaVariance;}

		const BioModelParamType TCPLQModel::getRho(){return _rho;}
		...
		const int TCPLQModel::getParameterID(const std::string aParamName) const{
			if(aParamName=="alphaMean")
				...
				{
		rttbExceptionMacro(core::InvalidParameterException, << "Parameter name "<<aParamName<<" invalid: it should be alphaMean or alphaVariance or alpha_beta or rho!");
