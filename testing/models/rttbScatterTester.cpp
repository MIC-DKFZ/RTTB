...
namespace rttb{
	...
	namespace testing{
		...
		ScatterTester::ScatterTester(models::CurveDataType aReferenceCurve, models::ScatterPlotType aCompareScatter, models::BioModelParamType aVariance){
			...
			for (iter = _compareScatter.begin(); iter != _compareScatter.end(); ++iter){
				...
				if (iterC != _referenceCurve.end()){
					...
					if (upper<(iter->second).first) {
						...
						if (difference < scatterError){continue;}
		...
		void ScatterTester::handleSuccess(void) const{
			...
			if (!_allowExceptions){
				...
				stream << std::endl << "Mean difference:  "<< _meanDifference << ", variance was "<<_variance<<std::endl;
			}
