...
/*!
// @file
// @version $Revision: 748 $ (last changed revision)
// @date    $Date: 2014-09-17 12:02:48 +0200 (Mi, 17 Sep 2014) $ (last change date)
	...
	namespace testing
	{
		...
		*/
		int OTBMaskAccessorTest(int argc, char* argv[])
		{
			...
			typedef masks::legacy::OTBMaskAccessor::MaskVoxelListPointer MaskVoxelListPointer;
			typedef masks::legacy::OTBMaskAccessor::MaskVoxelList MaskVoxelList;
			...
			//1) test constructors
			CHECK_NO_THROW(masks::legacy::OTBMaskAccessor(spMyStruct, spTestDoseAccessor->getGeometricInfo()));
			masks::legacy::OTBMaskAccessor testOTB1(spMyStruct, spTestDoseAccessor->getGeometricInfo());
			CHECK_EQUAL(spTestDoseAccessor->getGeometricInfo(), testOTB1.getGeometricInfo());

			CHECK_NO_THROW(masks::legacy::OTBMaskAccessor(spMyStruct, spTestDoseAccessor->getGeometricInfo()));
			masks::legacy::OTBMaskAccessor testOTB2(spMyStruct, spTestDoseAccessor->getGeometricInfo());
