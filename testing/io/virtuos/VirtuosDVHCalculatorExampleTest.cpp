  ...
  namespace testing{
    ...
    */
    ...
    int VirtuosDVHCalculatorExampleTest(int argc, char* argv[] )
    {
	  ...
	  io::virtuos::VirtuosPlanFileDoseAccessorGenerator doseAccessorGeneratorVirtuos(Virtuos_Dose_File,Virtuos_Plan_File);
	  DoseAccessorPointer doseAccessorVirtuos(doseAccessorGeneratorVirtuos.generateDoseAccessor());

	  StructureSetPointer rtStructureSetVirtuos=io::virtuos::VirtuosFileStructureSetGenerator(Virtuos_Structure_File,Virtuos_CT_File).generateStructureSet();
      ...
      for(int i=1;i<rtStructureSetVirtuos->getNumberOfStructures();i++){
        ...
        boost::shared_ptr<masks::OTBMaskAccessor> spOTBMaskAccessorVirtuos = 
          boost::make_shared<masks::OTBMaskAccessor>(rtStructureSetVirtuos->getStructure(i), doseAccessorVirtuos->getGeometricInfo());
        ...
        if(i==6)
          calc=new rttb::core::DVHCalculator(spMaskedDoseIterator,(rtStructureSetVirtuos->getStructure(i))->getUID(),doseAccessorVirtuos->getDoseUID(),0.367944);
        else
          calc=new rttb::core::DVHCalculator(spMaskedDoseIterator,(rtStructureSetVirtuos->getStructure(i))->getUID(),doseAccessorVirtuos->getDoseUID(),0.510107);
