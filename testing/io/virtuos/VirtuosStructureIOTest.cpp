  ...
  namespace testing{
	...
	*/
	...
	int VirtuosStructureIOTest(int argc, char* argv[] )
	{
	  ...
	  /* structure set2 */
	  StructureSetPointer rtStructureSet2=io::virtuos::VirtuosFileStructureSetGenerator(RTSTRUCT_FILENAME2.c_str(),CT_FILENAME2.c_str()).generateStructureSet();
	  //2) test structure import
	  ...
	  StructureSetPointer rtStructureSet3=io::virtuos::VirtuosFileStructureSetGenerator(RTSTRUCT_FILENAME2.c_str(),DOSE_FILENAME.c_str()).generateStructureSet();
	  ...
	  for(int s = 0; s < rtStructureSet3->getNumberOfStructures(); s++){
		  CHECK_EQUAL((rtStructureSet2->getStructure(s))->getLabel(),(rtStructureSet3->getStructure(s))->getLabel());
