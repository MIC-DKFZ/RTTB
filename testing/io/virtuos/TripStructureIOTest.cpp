		...
		*/
		...
		int TripStructureIOTest(int argc, char* argv[])
		{
			...
			StructureSetPointer rtStructureSet2 = io::virtuos::VirtuosFileStructureSetGenerator(
			        RTSTRUCT_FILENAME.c_str(), CT_FILENAME_TRIP.c_str()).generateStructureSet();
			//1) test structure import
			...
			{
				CHECK_EQUAL((rtStructureSet->getStructure(s))->getLabel(),
				            (rtStructureSet2->getStructure(s))->getLabel());
			...
			StructureSetPointer rtStructureSet3 = io::virtuos::VirtuosFileStructureSetGenerator(
			        RTSTRUCT_FILENAME.c_str(), DOSE_FILENAME.c_str()).generateStructureSet();
			...
			{
				CHECK_EQUAL((rtStructureSet3->getStructure(s))->getLabel(),
			...
			StructureSetPointer rtStructureSet4 = io::virtuos::VirtuosFileStructureSetGenerator(
			        RTSTRUCT_FILENAME.c_str(), DOSE_FILENAME_TRIP.c_str()).generateStructureSet();
			//1) test structure import, should be equal to structure imported on CT-image
