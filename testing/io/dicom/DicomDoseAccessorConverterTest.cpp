...
/*!
// @file
// @version $Revision: 856 $ (last changed revision)
// @date $Date: 2014-11-27 13:39:53 +0100 (Do, 27 Nov 2014) $ (last change date)
	...
	namespace testing
	{
		...
		*/
		...
		int DicomDoseAccessorConverterTest(int argc, char* argv[])
		{
			...
			//Check geometricinfo
			CHECK_CLOSE(doseAccessor_r->getGeometricInfo().getImagePositionPatient().x(), doseAccessor_w->getGeometricInfo().getImagePositionPatient().x(), 
				errorConstant);
			CHECK_CLOSE(doseAccessor_r->getGeometricInfo().getImagePositionPatient().y(), doseAccessor_w->getGeometricInfo().getImagePositionPatient().y(), 
				errorConstant);
			CHECK_CLOSE(doseAccessor_r->getGeometricInfo().getImagePositionPatient().z(), doseAccessor_w->getGeometricInfo().getImagePositionPatient().z(), 
				errorConstant);
			CHECK_CLOSE(doseAccessor_r->getGeometricInfo().getImageOrientationColumn().x(), doseAccessor_w->getGeometricInfo().getImageOrientationColumn().x(), 
				errorConstant);
			CHECK_CLOSE(doseAccessor_r->getGeometricInfo().getImageOrientationColumn().y(), doseAccessor_w->getGeometricInfo().getImageOrientationColumn().y(), 
				errorConstant);
			CHECK_CLOSE(doseAccessor_r->getGeometricInfo().getImageOrientationColumn().z(), doseAccessor_w->getGeometricInfo().getImageOrientationColumn().z(), 
				errorConstant);
			CHECK_CLOSE(doseAccessor_r->getGeometricInfo().getImageOrientationRow().x(), doseAccessor_w->getGeometricInfo().getImageOrientationRow().x(), 
				errorConstant);
			CHECK_CLOSE(doseAccessor_r->getGeometricInfo().getImageOrientationRow().y(), doseAccessor_w->getGeometricInfo().getImageOrientationRow().y(), 
				errorConstant);
			CHECK_CLOSE(doseAccessor_r->getGeometricInfo().getImageOrientationRow().z(), doseAccessor_w->getGeometricInfo().getImageOrientationRow().z(), 
				errorConstant);
			...
			for(unsigned int index=0; index<30; index++){
				CHECK_CLOSE(doseAccessor_r->getDoseAt(index), doseAccessor_w->getDoseAt(index), errorConstant);
				if(size/2 - index >=0 && size/2-index < size){
					CHECK_CLOSE(doseAccessor_r->getDoseAt(size/2 - index), doseAccessor_w->getDoseAt(size/2 - index), errorConstant);
				}
				CHECK_CLOSE(doseAccessor_r->getDoseAt(size-index-1), doseAccessor_w->getDoseAt(size-index-1), errorConstant);
			}
