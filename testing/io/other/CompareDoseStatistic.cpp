...
namespace rttb
{
	...
	namespace testing
	{
		...
		bool checkEqualDoseStatistic(DoseStatisticsPtr aDoseStatistc1, DoseStatisticsPtr aDoseStatistic2){
			bool result;
			result = lit::AreClose(aDoseStatistc1->getNumberOfVoxels(), aDoseStatistic2->getNumberOfVoxels(), errorConstant);
			...
			result = result && map_compare(aDoseStatistc1->getAllDx(), aDoseStatistic2->getAllDx());
			result = result && map_compare(aDoseStatistc1->getAllVx(), aDoseStatistic2->getAllVx());
			result = result && map_compare(aDoseStatistc1->getAllMaxOHx(), aDoseStatistic2->getAllMaxOHx());
			result = result && map_compare(aDoseStatistc1->getAllMinOCx(), aDoseStatistic2->getAllMinOCx());
			result = result && map_compare(aDoseStatistc1->getAllMOCx(), aDoseStatistic2->getAllMOCx());
			result = result && map_compare(aDoseStatistc1->getAllMOHx(), aDoseStatistic2->getAllMOHx());
			...
			bool map_compare(std::map<double, double> &lhs, std::map<double, double>  &rhs) {
				...
				for (std::map<double, double>::iterator i = rhs.begin(); i != rhs.end(); ++i)
				{
					double a = i->second;
					double b = getValue(lhs, i->first) ;
				if (abs(a - b ) > errorConstant){// errorConstant is 1e-3 because the double are castet to float when they are written
						return false;
					}
				}
				return true;
