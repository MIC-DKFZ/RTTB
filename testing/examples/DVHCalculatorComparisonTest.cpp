	...
	*/
    ...
    int DVHCalculatorComparisonTest(int argc, char* argv[] )
    {
      ...
      if(rtStructureSet->getNumberOfStructures()>0){
        for(int j=0;j<rtStructureSet->getNumberOfStructures();j++){
		  ...
		  boost::shared_ptr<masks::OTBMaskAccessor> spOTBMaskAccessor = 
			  boost::make_shared<masks::OTBMaskAccessor>(rtStructureSet->getStructure(j), doseAccessor1->getGeometricInfo());
          ...
          rttb::core::DVHCalculator calc(spMaskedDoseIterator,(rtStructureSet->getStructure(j))->getUID(),doseAccessor1->getDoseUID());
		  ...
		  rttb::io::other::DVHTxtFileReader dvhOriginalReader=rttb::io::other::DVHTxtFileReader(COMPARISON_DVH_FOLDER+dvhFileName+".txt");
      ...
      for(int j=0;j<rtStructSetMaskAccessorVec.size();j++){
		  //create corresponding MaskedDoseIterator
		  boost::shared_ptr<core::GenericMaskedDoseIterator> spMaskedDoseIteratorTmp = 
			  boost::make_shared<core::GenericMaskedDoseIterator>(rtStructSetMaskAccessorVec.at(j), doseAccessor2);
		  DoseIteratorPointer spMaskedDoseIterator (spMaskedDoseIteratorTmp);
		  
		  rttb::core::DVHCalculator calc(spMaskedDoseIterator,(rtStructureSet->getStructure(j))->getUID(),doseAccessor2->getDoseUID());
		  ...
		  for(it=dvhData.begin();it!=dvhData.end(),itOrig!=dvhOrigData.end();++it,++itOrig){
			  ...
			  difference = abs(*(itOrig)-*(it));
			  if (difference > 10){
				  std::cout <<"large difference: "<< abs(*(itOrig)-*(it))<<" = "<<*(itOrig)<<" - "<<*(it)<<std::endl;
			  }
			  if (difference < minDifference){
				  minDifference = difference;
			  }
      ...
      for(int j=0;j<rtStructSetMaskAccessorVec.size();j++){
		  ...
		  rttb::core::DVHCalculator calc(spMaskedDoseIterator,(rtStructureSet->getStructure(j))->getUID(),doseAccessor3->getDoseUID());
