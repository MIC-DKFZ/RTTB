	...
	namespace testing
	{
		...
		*/
		...
		int RTDVHCalculatorTest(int argc, char* argv[])
		{
			...
			// import dicom files
			CHECK_NO_THROW(DoseAcessorPointer spDoseAccessor(new core::DicomDoseAccessor(
			                   DOSE_FILENAME.c_str())));
			...
			if (rtstrset.getNumberOfStructures() > 0)
			{
				for (int j = 0; j < rtstrset.getNumberOfStructures(); j++)
				{
					...
					boost::shared_ptr<rttb::masks::OTBMaskAccessor> pOTBMaskAccessor =
					    boost::make_shared<rttb::masks::OTBMaskAccessor>(spRtstr, geoInf, rtdose);
					MaskAcessorPointer spMaskAccessor(pOTBMaskAccessor);
					boost::shared_ptr<rttb::core::GenericMaskedDoseIterator> spTestMaskedDoseIterator =
					    boost::make_shared<rttb::core::GenericMaskedDoseIterator>(spMaskAccessor,
					            spDoseAccessor); //using Mask
					...
					std::cout << "DVH(mask) max: " << dvh.getMaximum() << "in " << spRtstr->getVolume() << " cm^3" <<
					          std::endl;
				}
			}
			...
			if (rtstrset.getNumberOfStructures() > 0)
			{
				for (int j = 0; j < rtstrset.getNumberOfStructures(); j++)
				{
					...
					boost::shared_ptr<rttb::core::GenericMaskedDoseIterator> spTestMaskedDoseIterator =
					    boost::make_shared<rttb::core::GenericMaskedDoseIterator>(spMaskAccessor,
					            spDoseAccessor2); //using Mask
					DoseIteratorPointer mddit(spTestMaskedDoseIterator);
					boost::shared_ptr<rttb::core::GenericDoseIterator> spTestDoseIterator =
					    boost::make_shared<rttb::core::GenericDoseIterator>(spDoseAccessor2);
					...
					std::cout << "DVH(mask) max: " << dvh2.getMaximum() << "in " << spRtstr->getVolume() << " cm^3" <<
					          std::endl;
