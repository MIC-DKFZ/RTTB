		...
		namespace doseAcc
		{
			...
			ParseArgumentsForAppData(int argc, char** argv, ApplicationData& appData)
			{
				...
				cmdParser.AddArgument("-i", itksys::CommandLineArguments::SPACE_ARGUMENT,
				                      &(appData._interpolatorName),
				                      "Specifies the interpolator that should be used for mapping. Available options are: \"nn\": nearest neighbor, \"linear\": linear interpolation, \"rosu\" interpolation based on the concept of Rosu et al.. Default interpolator is \"linear\".");

				cmdParser.AddArgument("--weight1", itksys::CommandLineArguments::SPACE_ARGUMENT,
				                      &(appData._weightDose1),
				...
				cmdParser.AddArgument("-w1", itksys::CommandLineArguments::SPACE_ARGUMENT, &(appData._weightDose1),
				                      "Specifies the weight for dose 1. If not specified the weight is 1.0.");
				cmdParser.AddArgument("--weight2", itksys::CommandLineArguments::SPACE_ARGUMENT,
				                      &(appData._weightDose2),
				...
				cmdParser.AddArgument("--registration", itksys::CommandLineArguments::SPACE_ARGUMENT,
				                      ...
				                      "Specifies name and location of the registration file that should be used to map dose 2 before accumulating it with dose 1. Default is no mapping, thus direct accumulation. The registration should be stored as MatchPoint registration.");
				cmdParser.AddArgument("-r", itksys::CommandLineArguments::SPACE_ARGUMENT, &(appData._regFileName),
				                      "Specifies name and location of the registration file that should be used to map dose 2 before accumulating it with dose 1. Default is no mapping, thus direct accumulation. The registration should be stored as MatchPoint registration.");

				cmdParser.AddArgument("--loadStyle1", itksys::CommandLineArguments::MULTI_ARGUMENT,
				                      &(appData._dose1LoadStyle),
				                      "Indicates the load style that should be used for dose 1. Available styles: \"dicom\": normal dicom dose (default); \"virtuos\": load of a virtuos dose (This style is a multi argument. The second argument specifies the virtuos plan file, e.g. : \"--loadStyle1 virtuos myFavorite.pln\"); \"itk\": use itk image loading; \"helax\": load a helax dose (choosing this style, the dose path should only be a directory).");

				cmdParser.AddArgument("--loadStyle2", itksys::CommandLineArguments::MULTI_ARGUMENT,
				                      &(appData._dose2LoadStyle),
				                      "Indicates the load style that should be used for dose 2. Available styles: \"dicom\": normal dicom dose (default); \"virtuos\": load of a virtuos dose (This style is a multi argument. The second argument specifies the virtuos plan file, e.g. : \"--loadStyle2 virtuos myFavorite.pln\"); \"itk\": use itk image loading; \"helax\": load a helax dose (choosing this style, the dose path should only be a directory).");

				cmdParser.AddArgument("--operator", itksys::CommandLineArguments::SPACE_ARGUMENT,
				                      &(appData._operator),
				                      "Specifies the operator used. Available operators are '+' and '*'. If not specified the operator is '+'.");
				cmdParser.AddArgument("-o", itksys::CommandLineArguments::SPACE_ARGUMENT, &(appData._operator),
				                      "Specifies the operator used. Available operators are '+' and '*'. If not specified the operator is '+'.");
				...
				if (appData._showHelp)
				{
					...
					std::cout <<
					          " DoseAcc dose1.mhd dose2.mhd result.mhd --loadStyle1 itk --loadStyle2 itk -w1 2 -r reg.mapr" <<
					          std::endl << std::endl;
					std::cout <<
					          " This will accumulate \"dose1.mhd\" and \"dose2.mhd\" by using \"reg.mapr\" to map dose 2. For the accumulation, dose 1 will be multiplied by 2. The resulting dose will be stored in \"result.mhd\"."
					          << std::endl << std::endl;
					std::cout <<
					          " DoseAcc dose1.dcm dose2.dos.gz result.mhd --loadStyle2 virtuos dose2.pln -r reg.mapr" <<
					          std::endl << std::endl;
					std::cout <<
					          " This will accumulate \"dose1.dcm\" (using default dicom io) and \"dose2.dos.gz\" (using virtuos io and plan file dose2.pln) by using \"reg.mapr\" to map dose 2. The resulting dose will be stored in \"result.mhd\"."
					          << std::endl;
					return 1;
				}
